 ...
 * File: Higuchi2D_RGBDiff_SqrDiff.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class Higuchi2D_RGBDiff_SqrDiff implements Higuchi2DMethods{
	 ...
	 */
	public Higuchi2D_RGBDiff_SqrDiff(RandomAccessibleInterval<?> rai, int numK, boolean skipZeroes)  {
		this.rai   = rai;
		this.numK  = numK;
		this.skipZeroes = skipZeroes;
	}

	public Higuchi2D_RGBDiff_SqrDiff() {
	}
...
//						    sumDiff_Red = Math.abs(Z_red-C_red)*Math.abs(A_red-C_red)+
//				        		          Math.abs(Z_red-E_red)*Math.abs(A_red-E_red);
//						        
//						    sumDiff_Green = Math.abs(Z_green-C_green)*Math.abs(A_green-C_green)+
//			        		                Math.abs(Z_green-E_green)*Math.abs(A_green-E_green);
//						        
//						     sumDiff_Blue = Math.abs(Z_blue-C_blue)*Math.abs(A_blue-C_blue)+
//						        		    Math.abs(Z_blue-E_blue)*Math.abs(A_blue-E_blue);
						        
					        double rgbDiffZ = Math.abs(Z_red-Z_green) + Math.abs(Z_green-Z_blue) + Math.abs(Z_blue-Z_red);
					        double rgbDiffC = Math.abs(C_red-C_green) + Math.abs(C_green-C_blue) + Math.abs(C_blue-C_red);
					        double rgbdiffA = Math.abs(A_red-A_green) + Math.abs(A_green-A_blue) + Math.abs(A_blue-A_red);
					        double rgbDiffE = Math.abs(E_red-E_green) + Math.abs(E_green-E_blue) + Math.abs(E_blue-E_red);
				            
					        //RGBDiff_kfoldDiff
//					         double sumMultDiff = Math.abs(rgbDiffZ-rgbDiffC)*k +
//					            		             Math.abs(rgbDiffC-rgbdiffA)*k +
//					            		             Math.abs(rgbDiffZ-rgbDiffE)*k + 
//					            		             Math.abs(rgbDiffE-rgbdiffA)*k; 
					        
					        //RGBDiff_MultDiff
//					        double sumMultDiff = Math.abs(rgbDiffZ-rgbDiffC)*Math.abs(rgbDiffC-rgbdiffA) + 
//					            					 Math.abs(rgbDiffZ-rgbDiffE)*Math.abs(rgbDiffE-rgbdiffA);
				            //RGBDiff_SqrDiff
				            double sumMultDiff = Math.abs(rgbDiffZ-rgbDiffC) * Math.abs(rgbDiffZ-rgbDiffC) +
				            					 Math.abs(rgbDiffC-rgbdiffA) * Math.abs(rgbDiffC-rgbdiffA) +
				            					 Math.abs(rgbDiffZ-rgbDiffE) * Math.abs(rgbDiffZ-rgbDiffE) +
				            					 Math.abs(rgbDiffE-rgbdiffA) * Math.abs(rgbDiffE-rgbdiffA); 
				            
//				            L_vec[mm] += (sumMultDiff) * norm;
