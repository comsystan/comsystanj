...
package at.csa.csaj.plugin1d.autocorr;
...
@Plugin(type = ContextCommand.class,
	...
	@Menu(label = "Autocorrelation ", weight = 90)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SequenceAutoCorrelation<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Plugin1DAutoCorrelation<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
	...
	private void processSingleInputColumn (int s) {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			if (resultValues != null) { 
				...
				SequencePlotFrame pdf = new SequencePlotFrame(tableOut, cols, isLineVisible, "Autocorrelation sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
	...
	private void writeToTable(int sequenceNumber, double[] resultValues) {
		...
		if (resultValues == null) {
			...
			for (int r = 0; r < resultValues.length; r++ ) {
				tableOut.set(0, r, this.choiceRadioButt_SurrogateType);
				tableOut.set(1, r, this.choiceRadioButt_AutoCorrelationMethod);
				tableOut.set(sequenceNumber + numTableOutPreCols, r, resultValues[r]); //+ because of first text columns	
			}
...
//				}
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Plugin1DAutoCorrelation.class, true);
	}
}
