/*-
 * #%L
 * Project: ImageJ2 plugin for computing 3D lacunarity
 * File: Csaj3DLacunarity.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
package at.csa.csaj.plugin3d.frac.lacunarity;
...
import at.csa.csaj.plugin3d.frac.lacunarity.util.Lacunarity3DMethods;
import at.csa.csaj.plugin3d.frac.lacunarity.util.Lacunarity3D_Grey;
...
@Menu(label = "3D Lacunarity", weight = 70)})
//public class Csaj3DLacunarity<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj3DLacunarity<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	}
	...
	*/
	protected void startWorkflowForSingleVolume() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("Computing 3D Lacunarity, please wait... Open console window for further info.",
...
//			}
	...
	private void processSingleInputVolume() {
		...
		resultValuesTable = new double[numBoxes + 2]; //+2 because of weighted mean L and mean L
		...
		double[] lacunarities = process(rai); //rai is 3D
...
//		}
	...
	*/
	private void writeSingleResultToTable() { 
		...
		tableOut.set("<L>-R&P",   		   tableOut.getRowCount()-1, resultValuesTable[resultValuesTable.length - 2]); //
		tableOut.set("<L>-S&V",   		   tableOut.getRowCount()-1, resultValuesTable[resultValuesTable.length - 1]); //last entry	
		tableColLast = 5;
		
		int numParameters = resultValuesTable.length - 2; 
...
//		}
		...
		if (optShowPlot) {
			if ((imageType.equals("Grey")) || (imageType.equals("RGB"))) { //both are OK
				...
				RegressionPlotFrame doubleLogPlot = DisplayRegressionPlotXY(lnDataX, lnDataY, isLineVisible, plot_method, 
