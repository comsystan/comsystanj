/*-
 * #%L
 * Project: ImageJ signal plugin for computing DFA
 * File: SignalDFA.java
 * 
 * $Id$
 * $HeadURL$
...
import at.csa.csaj.sig.dfa.util.DFA;
...
//public class SignalDFA<T extends RealType<T>> implements Command {	//modal GUI
	...
	private static final String PLUGIN_LABEL            = "<html>Detrended fluctuation analysis</html>";
	...
	private static final int numWinSizeMax = 1000;
...
//	private final String labelSpace = SPACE_LABEL;
	...
	/** Executed whenever the {@link #spinInteger_WinSizeMax} parameter changes. */
	...
	protected void callbackProcessActiveColumn() {
		...
		//WaitingDialogWithProgressBar dlgProgress = new WaitingDialogWithProgressBar("<html>Computing DFA, please wait...<br>Open console window for further info.</html>");
		WaitingDialogWithProgressBar dlgProgress = new WaitingDialogWithProgressBar("Computing DFA, please wait... Open console window for further info.",
	...
	protected void callbackProcessAllColumns() {
		...
		//WaitingDialogWithProgressBar dlgProgress = new WaitingDialogWithProgressBar("<html>Computing DFA dimensions, please wait...<br>Open console window for further info.</html>");
		WaitingDialogWithProgressBar dlgProgress = new WaitingDialogWithProgressBar("Computing DFA, please wait... Open console window for further info.",
	...
	private void generateTableHeader() {
		...
		tableResult.add(new IntColumn("Window size"));
		...
		if (choiceRadioButt_SignalType.equals("Entire signal")){
			tableResult.add(new DoubleColumn("Alpha"));	
			...
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				...
				tableResult.add(new DoubleColumn("Alpha_Surr"));  //Mean surrogate value	
				...
				for (int s = 0; s < numSurrogates; s++) tableResult.add(new DoubleColumn("Alpha_Surr-#"+(s+1))); 
			...
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableResult.add(new DoubleColumn("Alpha-#" + n));	
			}
			for (int n = 1; n <= numSubsequentBoxes; n++) {
...
//			}
	...
	private void writeToTable(int signalNumber, double[] resultValues) {
		...
		tableResult.set(2, row, spinnerInteger_WinSizeMax); // WinSizeMax
...
//		}
		...
		if (signalType.equals("Entire signal")){	
			...
			if (signal1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
				...
				F = dfa.computeFluctuationFunction(signal1D, numWinSizeMax);
				regressionValues = dfa.computeAlpha(F, regMin, regMax);
				// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
				
				if (optShowPlot) {
					String preName = signalColumn.getHeader();
					showPlot(dfa.getLnDataX(), dfa.getLnDataY(), preName, col, regMin, regMax);
				}	
		...
		//********************************************************************************************************	
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				if (subSignal1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
					dfa = new DFA(logService);
					F = dfa.computeFluctuationFunction(subSignal1D, numWinSizeMax);
					regressionValues = dfa.computeAlpha(F, regMin, regMax);
					// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(SignalDFA.class, true);
	}
}
