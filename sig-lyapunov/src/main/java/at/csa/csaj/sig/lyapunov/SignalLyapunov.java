 ...
 * #L%
 */
package at.csa.csaj.sig.lyapunov;
...
import at.csa.csaj.sig.lyapunov.util.Lyapunov;
...
/**
 * A {@link Command} plugin computing <the Lyapunov exponent</a>
 * of a  signal.
 */
...
//public class SignalLyapunov<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class SignalLyapunov<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
	...
	*/
		...
		if (signalRange.equals("Entire signal")){	
			...
			if (signal1D.length > (numKMax * 2)) { // only data series which are large enough
				...
				if (algorithm.equals("Direct")) {
					...
					divergencesKantz = lya.calcDivergencesKantz(signal1D, embDim, tau, periodMean, numKMax, numPointPairs);
				...
				if (!surrType.equals("No surrogates")) { //Add surrogate analysis
					...
					for (int s = 0; s < numSurrogates; s++) {
						...
						if (algorithm.equals("Direct")) {
							...
							divergencesKantz = lya.calcDivergencesKantz(surrSignal1D, embDim, tau, periodMean, numKMax, numPointPairs);
							regressionValues = lya.calcRegressionsMean(divergencesKantz, regMin, regMax);
							// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
						}
	...
	private void showPlot(double[] dataX, double[] dataY, String preName, int col, int regMin, int regMax) {
		...
		RegressionPlotFrame doubleLogPlot = DisplayRegressionPlotXY(dataX, dataY, isLineVisible,
				"Plot - Lyapunov divergences", preName + "-" + tableInName, "Delay k", "<ln(divergence)>", "", regMin, regMax);
	...
	private void showPlots(double[] dataX, double[][] dataY, String preName, int col, int regMin, int regMax) {
		...
		RegressionPlotFrame doubleLogPlot = DisplayRegressionPlotXY(dataX, dataY, isLineVisible,
				"Plot - Lyapunov divergences", preName + "-" + tableInName, "Delay k", "<ln(divergence)>", legendLabels, regMin, regMax);
		doubleLogPlotList.add(doubleLogPlot);
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(SignalLyapunov.class, true);
	}
}
