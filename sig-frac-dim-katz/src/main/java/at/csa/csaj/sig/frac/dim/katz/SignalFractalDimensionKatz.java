/*-
 * #%L
 * Project: ImageJ signal plugin for computing fractal dimension with Katz algorithm.
...
public class SignalFractalDimensionKatz<T extends RealType<T>> extends InteractiveCommand implements Command, Previewable { // non blocking  GUI
//public class SignalFractalDimensionKatz<T extends RealType<T>> implements Command {	//modal GUI
...
//	private final String labelBackgroundOptions = BACKGROUNDOPTIONS_LABEL;
	...
	protected void callbackProcessSingleColumn() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing Katz dimensions, please wait...<br>Open console window for further info.</html>");
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SignalRange.equals("Entire signal")){
			tableResult.add(new DoubleColumn("Dk"));	
			
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				//do nothing	
			} else { //Surrogates
				tableResult.add(new DoubleColumn("Dk_Surr")); //Mean surrogate value	
			
				for (int s = 0; s < numSurrogates; s++) tableResult.add(new DoubleColumn("DkSurr-#"+(s+1))); 
			}	
		} 
		else if (choiceRadioButt_SignalRange.equals("Subsequent boxes")){
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableResult.add(new DoubleColumn("Dk-#" + n));	
			}
		}
