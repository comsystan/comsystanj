/*-
 * #%L
 * Project: ImageJ signal plugin for computing surrogates
 * File: SignalSurrogate.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
package at.csa.csaj.sig.surrogate;
...
/**
 * A {@link Command} plugin computing <surrogates</a>
 * of a signal.
 */
@Plugin(type = InteractiveCommand.class, headless = true, menuPath = "Plugins>ComsystanJ>Signal>Surrogates")
public class SignalSurrogate<T extends RealType<T>> extends InteractiveCommand implements Command, Previewable { // non blocking  GUI
//public class SignalSurrogate<T extends RealType<T>> implements Command {	//modal GUI
...
//		sliceLabels = new String[(int) numColumns];
	...
	private void generateTableHeader() {
		...
		if      (this.choiceRadioButt_SurrogateType.equals("Shuffle"))      preString = "Shuffle";  //+ this.spinnerInteger_NumSurrogates;
		else if (this.choiceRadioButt_SurrogateType.equals("Gaussian"))     preString = "Gaussian"; //+ this.spinnerInteger_NumSurrogates;
		else if (this.choiceRadioButt_SurrogateType.equals("Random phase")) preString = "RndPhase"; //+ this.spinnerInteger_NumSurrogates;
		else if (this.choiceRadioButt_SurrogateType.equals("AAFT"))         preString = "AAFT";     //+ this.spinnerInteger_NumSurrogates;
	...
	private void processSingleInputColumn (int s) throws InterruptedException {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			...
			String signalTitle = "Surrogate - " + this.choiceRadioButt_SurrogateType;
			String xLabel = "#";
			String yLabel = "Value";
...
//		}
		...
		//if (analysisType.equals("Entire signal")){	//only this option is possible for FFT
			...
			if (surrType.equals("Shuffle")) {
				 signal1D = surrogate.calcSurrogateShuffle(signal1D);		
		...
		//********************************************************************************************************	
		//} else if (analysisType.equals("Subsequent boxes")){ //not for Surrogate
		
		//********************************************************************************************************			
		//} else if (analysisType.equals("Gliding box")){ //not for Surrogate
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(SignalSurrogate.class, true);
	}
}
