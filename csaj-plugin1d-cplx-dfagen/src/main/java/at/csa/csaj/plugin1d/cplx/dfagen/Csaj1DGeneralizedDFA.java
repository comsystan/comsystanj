 ...
 * File: Csaj1DGeneralizedDFA.java
 * 
 * $Id$
 * $HeadURL$
 * 
 ...
 * #L%
 */
package at.csa.csaj.plugin1d.cplx.dfagen;
...
import at.csa.csaj.plugin1d.cplx.dfagen.util.GeneralizedDFA;
...
@Plugin(type = ContextCommand.class, 
	headless = true,
	label = "Generalized Detrended fluctuation analysis",
	...
	@Menu(label = "Generalized DFA ", weight = 151)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class Csaj1DGeneralizedDFA<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj1DGeneralizedDFA<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel

	private static final String PLUGIN_LABEL            = "<html><b>Generalized DFA</b></html>";
	private static final String SPACE_LABEL             = "";
	private static final String REGRESSION_LABEL        = "<html><b>Regression parameters</b></html>";
	private static final String MULTIFRACTAL_LABEL      = "<html><b>Multifractal options</b></html>";
...
//	private final String labelSpace = SPACE_LABEL;
	...
	@Parameter(label = "Window size Max", description = "Window size", style = NumberWidget.SPINNER_STYLE, min = "3", max = "9999999999999999999", stepSize = "1",
			   persist = false, // restore  previous value  default  =  true
			   initializer = "initialWinSizeMax", callback = "callbackWinSizeMax")
	private int spinnerInteger_WinSizeMax;

	@Parameter(label = "Regression Min", description = "Minimum x value of linear regression", style = NumberWidget.SPINNER_STYLE, min = "4", max = "9999999999999999999", stepSize = "1",
			   persist = false, //restore previous value default = true
			   initializer = "initialRegMin", callback = "callbackRegMin")
	private int spinnerInteger_RegMin = 1;

	@Parameter(label = "Regression Max", description = "Maximum x value of linear regression", style = NumberWidget.SPINNER_STYLE, min = "5", max = "9999999999999999999", stepSize = "1",
			   persist = false, //restore previous value default = true
	...
	//-----------------------------------------------------------------------------------------------------
	@Parameter(label = " ", visibility = ItemVisibility.MESSAGE, persist = false)
	private final String labelMutlifractal = MULTIFRACTAL_LABEL;
	
	@Parameter(label = "Min q", description = "minimal Q for multifractal DFA", style = NumberWidget.SPINNER_STYLE, min = "-1000", max = "1000", stepSize = "1",
			   persist = false, // restore  previous value  default  =  true
			   initializer = "initialMinQ", callback = "callbackMinQ")
	private int spinnerInteger_MinQ;

	@Parameter(label = "Max q", description = "maximal Q for multifractal DFA", style = NumberWidget.SPINNER_STYLE, min = "-1000", max = "1000", stepSize = "1",
			   persist = false, //restore previous value default = true
	...
	@Parameter(label = "Box length",
			   description = "Length of subsequent or gliding box - Shoud be at least three times WinSizeMax",
			   style = NumberWidget.SPINNER_STYLE, 
			   min = "2",
			   max = "9999999999999999999",
			   stepSize = "1",
...
//	private Button buttonProcessAllColumns;
	...
	protected void initialWinSizeMax() {
		//numbWinSizeMax = (int) Math.floor(tableIn.getRowCount() / 3.0);
		spinnerInteger_WinSizeMax = 8;
	}
	protected void initialRegMin() {
		spinnerInteger_RegMin = 4;
	}
	...
	/** Executed whenever the {@link #spinnerInteger_WinSizeMax} parameter changes. */
	protected void callbackWinSizeMax() {
		...
		if (spinnerInteger_RegMax > spinnerInteger_WinSizeMax) {
			spinnerInteger_RegMax = spinnerInteger_WinSizeMax;
		}
		if (spinnerInteger_RegMin >= spinnerInteger_RegMax - 2) {
			spinnerInteger_RegMin = spinnerInteger_RegMax - 2;
		}
		logService.info(this.getClass().getName() + " Window size set to " + spinnerInteger_WinSizeMax);
	}
	...
	*/
	protected void startWorkflowForSingleColumn() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("Computing Generalized DFA, please wait... Open console window for further info.",
...
//			}
	...
	private void processSingleInputColumn (int c) {
		...
		logService.info(this.getClass().getName() + " Alpha: " + resultValues[0]);
	...
	private void writeToTable(int rowNumber, int sequenceNumber, double[] resultValues) {
		...
		tableOut.set(7, row, spinnerInteger_WinSizeMax); // WinSizeMax
	...
	*/
	private double[] process(DefaultGenericTable dgt, int col) { //  c column number
		...
		double[] resultValues = new double[numQ]; // Gen alphas
		for (int r = 0; r <resultValues.length; r++) resultValues[r] = Double.NaN;
		
//		double[]totals = new double[numWinSizeMax];
//		double[]eps = new double[numWinSizeMax];
//		// definition of eps
//		for (int kk = 0; kk < numWinSizeMax; kk++) {
//			eps[kk] = kk + 1;		
...
//		}
		...
		if (sequenceRange.equals("Entire sequence")){	
			if (surrType.equals("No surrogates")) {
				resultValues = new double[numQ]; // Dim, R2, StdErr	
				for (int r = 0; r < resultValues.length; r++) resultValues[r] = Double.NaN;
				...
				if (sequence1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
					...
					if (optShowPlot) {
						...
						//showPlot(genDfa.getLnDataX(), genDfa.getLnDataY(), preName, col, regMin, regMax);
						
						boolean isLineVisible = false; //?
						String axisNameX = "ln(Window size)";
						String axisNameY = "ln(F)";
					
						String[] legendLabels = new String[numQ];
						for (int q = 0; q < numQ; q++) {
							legendLabels[q] = "q=" + (q + minQ); 
						}
						RegressionPlotFrame doubleLogPlot = DisplayMultipleRegressionPlotXY(genDfa.getLnDataX(), genDfa.getLnDataY(), isLineVisible, "Double Log Plot - Generalized DFA", 
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				if (subSequence1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
					...
					if ((optShowPlot) && (i==0)){ //show only first plot
						...
						showPlot(genDfa.getLnDataX(), genDfa.getLnDataY()[0], preName, col, regMin, regMax);
					}
		...
		//********************************************************************************************************			
			...
			resultValues = new double[(int) (1*numGlidingBoxes)]; // Dim  == one * number of boxes	
			for (int r = 0; r<resultValues.length; r++) resultValues[r] = Double.NaN;
			subSequence1D = new double[(int) numBoxLength];
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Csaj1DGeneralizedDFA.class, true);
	}
}
