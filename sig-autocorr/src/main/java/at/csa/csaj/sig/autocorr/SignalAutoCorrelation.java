/*-
 * #%L
 * Project: ImageJ signal plugin for computing the Autocorrelation
 ...
 * of a signal.
 */
@Plugin(type = InteractiveCommand.class, headless = true, menuPath = "Plugins>ComsystanJ>Signal>Autocorrelation")
public class SignalAutoCorrelation<T extends RealType<T>> extends InteractiveCommand implements Command, Previewable { // non blocking  GUI
...
//	private final String labelSpace = SPACE_LABEL;
	...
	@Parameter(label = "Limit to Max lag", persist = false,
		       callback = "callbackLimitToMaxLag")
	private boolean booleanLimitToMaxLag;
	
	@Parameter(label = "(Limit) Max lag", description = "maximal (time) lag", style = NumberWidget.SPINNER_STYLE, min = "1", max = "1000", stepSize = "1",
			   persist = false, // restore  previous value  default  =  true
			   initializer = "initialNumMaxLag", callback = "callbackNumMaxLag")
...
//	}
	...
	protected void callbackProcessActiveColumn() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing Autocorrelation, please wait...<br>Open console window for further info.</html>");
	...
	protected void callbackProcessAllColumns() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing AutoCcrrelation, please wait...<br>Open console window for further info.</html>");
		dlgProgress = new WaitingDialogWithProgressBar("Computing Autocorrelation, please wait... Open console window for further info.",
...
//		sliceLabels = new String[(int) numColumns];
	...
	private void generateTableHeader() {
		...
		tableResult.add(new GenericColumn("AC-Method"));
		for (int c = 0; c < numColumns; c++) {
			tableResult.add(new GenericColumn("AC-" + tableIn.getColumnHeader(c)));
		}	
		tableResult.appendRows((int) numMaxLag);
	}
	...
	private void writeToTable(int signalNumber, double[] resultValues) {
		...
		for (int r = 0; r < resultValues.length; r++ ) {
			...
			tableResult.set(1, r, this.choiceRadioButt_AutoCorrelationMethod);
...
//					magnitude[i] = complx[i].abs() * complx[i].abs(); //magnitude
//				}
//				//magnitude[0] = 0;
