 ...
 * File: Csaj2DRenyiHeterogeneitiesCmd.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class Csaj2DRenyiHeterogeneitiesCmd<T extends RealType<T>> extends ContextCommand implements Previewable {
	
	private static final String PLUGIN_LABEL            = "<html><b>Computes Renyi heterogeneities</b></html>";
...
//			//This dose not work with DisplayService because the JFrame is not "registered" as an ImageJ display	
	...
	private CsajContainer_ProcessMethod process(RandomAccessibleInterval<?> rai, int plane) { //plane plane (Image) number
		...
		//Collect some Renyi heterogeneities for display
		double[] entList;
		double[] qList;		
		if (optShowRenyiPlot) {	
			int offset = 0; 
