 ...
 * File: Higuchi2D_Grey_DirDiff.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class Higuchi2D_Grey_DirDiff implements Higuchi2DMethods{
	 ...
	 */
	public Higuchi2D_Grey_DirDiff(RandomAccessibleInterval<?> rai, int numK, boolean skipZeroes) {
		this.rai = rai;
		this.numK = numK;
		this.skipZeroes = skipZeroes;
	}

	public Higuchi2D_Grey_DirDiff() {
	}
	...
	public double[] calcTotals() {
		...
		System.out.println("Higuchi2D_DirDiff_HA: Preparing integer array...");
...
//							//L_vec[mm] += Math.abs(8*A-B-C-D-E-F-G-H-Z)/8.0  * norm;
//							//L_vec[mm] += Math.abs(3*A-C-E-Z)/3.0  * norm;
//							//L_vec[mm] += (2*Math.abs(A-Z)*k + Math.abs(Z-C)*k + Math.abs(A-C)*k+ Math.abs(Z-E)*k + Math.abs(E-A)*k)/6.0 *2*k * norm;
							
							if (!skipZeroes) { //no skipping
								//L_vec[mm] += Math.abs(3*A-C-E-Z)/3.0;//  * norm; before 2022-04-12
								L_vec[mm] += (Math.abs(A-C)+Math.abs(A-E))/2.0;//  * norm; since 2022-04-12
