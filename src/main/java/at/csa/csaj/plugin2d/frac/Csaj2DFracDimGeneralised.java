...
//public class Csaj2DFracDimGeneralised<T extends RealType<T>> extends InteractiveCommand { //non blocking GUI
public class Csaj2DFracDimGeneralised<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//				imgDil = (Img<FloatType>) opService.filter().convolve(imgBin, avgKernel, oobf);
	...
	public static void main(final String... args) throws Exception {
		...
		//ij.command().run(Csaj2DFracDimGeneralised.class, true).get().getOutput("image");
		ij.command().run(Csaj2DFracDimGeneralised.class, true);
	}
}
