 ...
 * File: Higuchi2D_Grey_MultDiff2.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
/**	Higuchi-Dimension for 2D images, method of multiplicated differences
 * according to Spasic On 2D generalization of Higuchi's fractal dimension chaos, Slitions &Fractals 69 (2014) 179-187
 *
...
public class Higuchi2D_Grey_MultDiff2 implements Higuchi2DMethods{
	 ...
	 */
	public Higuchi2D_Grey_MultDiff2(RandomAccessibleInterval<?> rai, int numK, boolean skipZeroes) {
		this.rai = rai;
		this.numK = numK;
		this.skipZeroes = skipZeroes;
	}

	public Higuchi2D_Grey_MultDiff2() {
	}
	...
	public double[] calcTotals() {
		...
		//System.out.println("Higuchi2D_MultiDiff2_HA: Preparing integer array...");
...
//		}
		...
		for (int k = 1; k <= numK; k++) {
			...
			double[] L_vec = new double[k];
			int mm = 0;
			for (int m1 = 1; m1 <= k ; m1++) {
				//for (int m2 = 1; m2 <= k ; m2++) {
				int m2 = m1; //according to Spasic 
...
//								int H=array2D[x_h][y_h];		
							...
							if (!skipZeroes) { //no skipping						
								L_vec[mm] += ((Math.abs(A-C)*Math.abs(C-Z)+
										   Math.abs(A-E)*Math.abs(E-Z))/2.0);// * norm;	
			...
			//Mean over all m's
			Double L_Sum = 0.0;
			for (int mm1 = 0; mm1 < k; mm1++) {			
				L_Sum =  L_Sum + L_vec[mm1] / ((double)(k)*(double)(k));  //last k outside the brackets
			}
			totals[k-1] = L_Sum/(double)(k);
		} //k
		return totals;
	}
