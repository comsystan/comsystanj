 ...
 */
public class Csaj2DFracDimBoxCountingDialog extends CsajDialog_2DPluginWithRegression {
    ...
    private JRadioButton radioButtonDBC;
    private JRadioButton radioButtonRDBC;
	 ...
	 * Create the dialog.
	 */
	public Csaj2DFracDimBoxCountingDialog(Context context, Dataset datasetIn) {
		...
		//Title of plugin
		//Overwrite
		setTitle("2D Box counting dimension");
		...
		radioButtonDBC    = new JRadioButton("DBC");
		radioButtonRDBC   = new JRadioButton("RDBC");
		...
		radioButtonDBC.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent arg0) {
				if (radioButtonDBC.isSelected())  choiceRadioButt_ColorModelType = radioButtonDBC.getText();
				logService.info(this.getClass().getName() + " Color model type set to " + choiceRadioButt_ColorModelType);
				if (booleanProcessImmediately) btnProcessSingleImage.doClick();
			}
		});
		radioButtonRDBC.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent arg0) {
				if (radioButtonRDBC.isSelected())  choiceRadioButt_ColorModelType = radioButtonRDBC.getText();
				logService.info(this.getClass().getName() + " Color model type set to " + choiceRadioButt_ColorModelType);
				if (booleanProcessImmediately) btnProcessSingleImage.doClick();
			}
		});
		buttonGroupColorModelType.add(radioButtonBinary);
		buttonGroupColorModelType.add(radioButtonDBC);
		buttonGroupColorModelType.add(radioButtonRDBC);
	    ...
	    panelColorModelType.add(radioButtonDBC); 
	    panelColorModelType.add(radioButtonRDBC); 
		...
		int numBoxes = Csaj2DFracDimBoxCountingCommand.getMaxBoxNumber(datasetIn.dimension(0), datasetIn.dimension(1));
	...
	public void processCommand() {
		...
		Future<CommandModule> future = commandService.run(Csaj2DFracDimBoxCountingCommand.class, false,
		...
		tableOutName = Csaj2DFracDimBoxCountingCommand.TABLE_OUT_NAME;
