...
/**
 * This is an ImageJ {@link InteractiveCommand} plugin to open multiple images as a volume.
 * <p>
 * 
 * </p>
 * <p>
 ...
 *
 */
public class Csaj3DOpener<T extends RealType<T>> extends InteractiveCommand {
	...
	protected void startWorkflow()  {
		...
		if (files.length == 1) {
			...
			try {
				...
				logService.error(this.getClass().getName() + " ERROR: IOException, it was not possible to load an image volume");
				//e.printStackTrace();
				dlgProgress.setVisible(false);
				dlgProgress.dispose();
				return;
			}  catch (NumberFormatException nfe) {
				logService.error(this.getClass().getName() + " ERROR: NumberFormatException, it was not possible to load an image volume");
				//nfe.printStackTrace();
				dlgProgress.setVisible(false);
				dlgProgress.dispose();
				return;
			} catch (ClassCastException cce) {
				logService.error(this.getClass().getName() + " ERROR: ClassCastException, it was not possible to load an image volume");
				//cce.printStackTrace();
				dlgProgress.setVisible(false);
				dlgProgress.dispose();
				return;
