...
public class Csaj3Dto1DScanCmd<T extends RealType<T>> extends ContextCommand implements Previewable {

	private static final String PLUGIN_LABEL            = "3D to 1D scan";
	...
	public static final String TABLE_OUT_NAME = "Table - 3D to 1D scan";
   ...
   @Parameter(label = " ", visibility = ItemVisibility.MESSAGE, persist = false)
    ...
    @Parameter(label = "Scan type",
		    description = "Type of 3D to 1D scanning",
		    style = ChoiceWidget.RADIO_BUTTON_VERTICAL_STYLE,
  		    choices = {"Hilbert", "Meander", "Random"}, 
...
//			}
	...
	private void processSingleInputVolume() {
		...
		writeToSingleColumnTable(containerPM); //write always to the first row
	...
	private void generateSingleColumnTableHeader() {
	    ...
	    int depth  = (int)datasetIn.dimension(2);
	    int numberOfSequencePoints = width * height *depth;
	 ...
	 */
	private void writeToSingleColumnTable(CsajContainer_ProcessMethod containerPM) { 

		String scanType = choiceRadioButt_ScanType;
		int numberOfValues = containerPM.item1_Values.length;
	  
		    
	    String columnHeader = "1Dscan-" + scanType;
