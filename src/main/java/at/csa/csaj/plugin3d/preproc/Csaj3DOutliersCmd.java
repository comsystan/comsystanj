...
public class Csaj3DOutliersCmd<T extends RealType<T>> extends ContextCommand implements Previewable {

	private static final String PLUGIN_LABEL            = "3D Outliers";
	...
	public static final String VOLUME_OUT_NAME = "Outliers volume";
...
//			datasetOut.setColorTable(ColorTables.BLUE,  2);
	...
	*/
	protected void startWorkflowForSingleVolume() {
		...
		dlgProgress = new CsajDialog_WaitingWithProgressBar("3D Outliers detection, please wait... Open console window for further info.",
...
//		}
	...
	private <C extends ComplexType<C>, R extends RealType<R>> RandomAccessibleInterval<R> process(RandomAccessibleInterval<R> rai) { //volume, grey or RGB
		...
		if (detectionType.equals("Z-score")) {
			...
			if (imageType.equals("Grey") || imageType.equals("RGB")) {
				...
				while (cursor.hasNext()) {
					cursor.fwd();
					stats.addValue(((UnsignedByteType) cursor.get()).get());
				}
				...
				if (extractType.equals("Eliminate outliers")) {
					...
					while (cursor.hasNext()) {
						...
						if (Math.abs(pixelValue - mean)/stdev <= zScore) {
							//do nothing
						} else {
		...
		else if (detectionType.equals("IQR")) {//Inter-quartile range //https://www.scribbr.com/statistics/outliers/
			...
			if (imageType.equals("Grey") || imageType.equals("RGB")) {
				...
				if (extractType.equals("Eliminate outliers")) {
					...
					while (cursor.hasNext()) {
						...
						if ((pixelValue <= upperFence) && (pixelValue >= lowerFence)) {
							//do nothing
						} else {
							((UnsignedByteType) cursor.get()).set(0);
						}
	...
	public static void main(final String... args) throws Exception {
		...
		ij.ui().show(image);
		// execute the command, waiting for the operation to finish.
