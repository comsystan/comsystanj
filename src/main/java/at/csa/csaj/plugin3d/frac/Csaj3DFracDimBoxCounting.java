 ...
 * File: Csaj3DFracDimBoxCounting.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
@Menu(label = "3D Box counting dimension")})
//public class Csaj3DFracDimBoxCounting<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj3DFracDimBoxCounting<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	}
	...
	*/
	protected void startWorkflowForSingleVolume() {
		...
		dlgProgress = new Dialog_WaitingWithProgressBar("Computing 3D Box counting dimension, please wait... Open console window for further info.",
...
//			}
	...
	*/
	private Container_ProcessMethod process(RandomAccessibleInterval<?> rai) { //3Dvolume
		...
		if (optShowPlot) {
			if ((imageType.equals("Grey")) || (imageType.equals("RGB"))) { //both are OK
				...
				Plot_RegressionFrame doubleLogPlot = DisplayRegressionPlotXY(lnEps, lnTotals, isLineVisible, plot_method, 
	...
	public static void main(final String... args) throws Exception {
		...
		// ij.command().run(Csaj3DFracDimBoxCounting.class,
		// true).get().getOutput("image");
		ij.command().run(Csaj3DFracDimBoxCounting.class, true);
	}
}
