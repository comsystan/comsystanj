 ...
 * File: Csaj3DFracDimGeneralisedCmdUI.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
@Plugin(type = ContextCommand.class,
		headless = true,
		label = "3D Generalised dimension",
		...
		@Menu(label = "3D Generalised dimension(New dialog)")})

public class Csaj3DFracDimGeneralisedCmdUI extends ContextCommand implements Previewable{
	...
	private Csaj3DFracDimGeneralisedDialog dialog = null;
	...
	public void run() {	
		SwingUtilities.invokeLater(() -> {
			if (dialog == null) {
				dialog = new Csaj3DFracDimGeneralisedDialog(context(), datasetIn);
			}
