 ...
 * File: Csaj3DFracDimTugOfWar.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
//public class Csaj3DFracDimTugOfWar<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj3DFracDimTugOfWar<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	}
	...
	*/
	protected void startWorkflowForSingleVolume() {
		...
		dlgProgress = new Dialog_WaitingWithProgressBar("Computing 3D Tug of war dimension, please wait... Open console window for further info.",
...
//			}
	...
	private void writeToTable(Container_ProcessMethod containerPM) { 
		...
		tableOut.set("3D Dtow",      row, containerPM.item1_Values[1]);
...
private int getPrimeNumber(){
	...
	private void showPlot(double[] lnDataX, double[] lnDataY, String preName, int numRegStart, int numRegEnd) {
		if (imageType.equals("Grey")) {
			...
			Plot_RegressionFrame doubleLogPlot = DisplayRegressionPlotXY(lnDataX, lnDataY, isLineVisible,
					"Double log plot - 3D Tug of war dimension", preName + datasetName, "ln(Box width)", "ln(Count)", "", numRegStart, numRegEnd);
			doubleLogPlotList.add(doubleLogPlot);
		}
		if (!imageType.equals("Grey")) {
	...
	public static void main(final String... args) throws Exception {
		...
		// ij.command().run(Csaj3DFracDimTugOfWar.class,
		// true).get().getOutput("image");
		ij.command().run(Csaj3DFracDimTugOfWar.class, true);
	}
}
