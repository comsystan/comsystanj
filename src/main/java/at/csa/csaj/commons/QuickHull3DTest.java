...
public class QuickHull3DTest
{
	...
	private void randomlyPerturb (QuickHull3D_Point3d pnt, double tol)
	 {
	...
	public double[] randomDegeneratePoints (int num, int dimen)
	 { 
	   double[] coords= new double[num*3];
	   QuickHull3D_Point3d pnt = new QuickHull3D_Point3d();

	   QuickHull3D_Point3d base = new QuickHull3D_Point3d();
	   ...
	   if (dimen == 0)
	    ...
	    { QuickHull3D_Vector3d u = new QuickHull3D_Vector3d();
	    ...
	    { QuickHull3D_Vector3d nrm = new QuickHull3D_Vector3d();
	      nrm.setRandom (-1, 1, rand);
	      nrm.normalize();
	      for (int i=0; i<num; i++)
	       { // compute a random point and project it to the plane
		 QuickHull3D_Vector3d perp = new QuickHull3D_Vector3d();
...
"Error: " + faceIndices.length + " faces vs. " + checkFaces.length);
	   ...
	   QuickHull3D_Point3d[] pnts = hull.getVertices();
