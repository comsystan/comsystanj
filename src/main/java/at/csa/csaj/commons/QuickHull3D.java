 ...
 * {@link #build(QuickHull3D_Point3d[]) build} method. After
...
public class QuickHull3D
{
        ...
        protected QuickHull3D_Vertex[] pointBuffer = new QuickHull3D_Vertex[0];
        protected int[] vertexPointIndices = new int[0];
        private QuickHull3D_Face[] discardedFaces = new QuickHull3D_Face[3];

        private QuickHull3D_Vertex[] maxVtxs = new QuickHull3D_Vertex[3];
        private QuickHull3D_Vertex[] minVtxs = new QuickHull3D_Vertex[3];

        protected Vector faces = new Vector(16);
        protected Vector horizon = new Vector(16);

        private QuickHull3D_FaceList newFaces = new QuickHull3D_FaceList();
        private QuickHull3D_VertexList unclaimed = new QuickHull3D_VertexList();
        private QuickHull3D_VertexList claimed = new QuickHull3D_VertexList();
        ...
        private void addPointToFace (QuickHull3D_Vertex vtx, QuickHull3D_Face face)
         {
           vtx.face = face;
        ...
        private void removePointFromFace (QuickHull3D_Vertex vtx, QuickHull3D_Face face)
         {
           if (vtx == face.outside)
        ...
        private QuickHull3D_Vertex removeAllPointsFromFace (QuickHull3D_Face face)
         {
           if (face.outside != null)
            { 
              QuickHull3D_Vertex end = face.outside;
         ...
         * coplanar.
         */
        public QuickHull3D (QuickHull3D_Point3d[] points)
           throws IllegalArgumentException
         {
           build (points, points.length);
         }

        private QuickHull3D_HalfEdge findHalfEdge (QuickHull3D_Vertex tail, QuickHull3D_Vertex head)
         { 
           // brute force ... OK, since setHull is not used much
           for (Iterator it=faces.iterator(); it.hasNext(); ) 
            { QuickHull3D_HalfEdge he = ((QuickHull3D_Face)it.next()).findEdge (tail, head);
              if (he != null)
               { return he; 
               }
            }
        ...
        protected void setHull (double[] coords, int nump,
                                int[][] faceIndices, int numf)
         {
           ...
           for (int i=0; i<numf; i++)
            { QuickHull3D_Face face = QuickHull3D_Face.create (pointBuffer, faceIndices[i]);
              QuickHull3D_HalfEdge he = face.he0;
              do
               { QuickHull3D_HalfEdge heOpp = findHalfEdge (he.head(), he.tail());
                 if (heOpp != null)
        ...
        private void printPoints (PrintStream ps)
         {
           for (int i=0; i<numPoints; i++)
            { QuickHull3D_Point3d pnt = pointBuffer[i].pnt;
         ...
         * coplanar.
         */
        public void build (QuickHull3D_Point3d[] points)
           throws IllegalArgumentException
         {
           build (points, points.length);
         }
         ...
         */
        public void build (QuickHull3D_Point3d[] points, int nump)
           throws IllegalArgumentException
         {
           if (nump < 4)
            { throw new IllegalArgumentException (
        ...
        public void triangulate ()
         {
           ...
           newFaces.clear();
           for (Iterator it=faces.iterator(); it.hasNext(); ) 
            { QuickHull3D_Face face = (QuickHull3D_Face)it.next();
              if (face.mark == QuickHull3D_Face.VISIBLE)
               { 
                 face.triangulate (newFaces, minArea);
                 // splitFace (face);
               }
            }
           for (QuickHull3D_Face face=newFaces.first(); face!=null; face=face.next)
            { faces.add (face);
            }
         }
...
//       }
        ...
        protected void initBuffers (int nump)
         {
           if (pointBuffer.length < nump)
            { QuickHull3D_Vertex[] newBuffer = new QuickHull3D_Vertex[nump];
              vertexPointIndices = new int[nump];
              for (int i=0; i<pointBuffer.length; i++)
               { newBuffer[i] = pointBuffer[i]; 
               }
              for (int i=pointBuffer.length; i<nump; i++)
               { newBuffer[i] = new QuickHull3D_Vertex(); 
               }
        ...
        protected void setPoints (double[] coords, int nump)
         { 
           for (int i=0; i<nump; i++)
            { 
              QuickHull3D_Vertex vtx = pointBuffer[i];
              vtx.pnt.set (coords[i*3+0], coords[i*3+1], coords[i*3+2]);
              vtx.index = i;
            }
         }

        protected void setPoints (QuickHull3D_Point3d[] pnts, int nump)
         { 
           for (int i=0; i<nump; i++)
            { 
              QuickHull3D_Vertex vtx = pointBuffer[i];
        ...
        protected void computeMaxAndMin ()
         {
           QuickHull3D_Vector3d max = new QuickHull3D_Vector3d();
           QuickHull3D_Vector3d min = new QuickHull3D_Vector3d();
...
"Input points appear to be coincident");
           ...
           QuickHull3D_Vertex[] vtx = new QuickHull3D_Vertex[4];
           ...
           QuickHull3D_Vector3d u01 = new QuickHull3D_Vector3d();
           QuickHull3D_Vector3d diff02 = new QuickHull3D_Vector3d();
           QuickHull3D_Vector3d nrml = new QuickHull3D_Vector3d();
           QuickHull3D_Vector3d xprod = new QuickHull3D_Vector3d();
           double maxSqr = 0;
...
"Input points appear to be colinear");
           ...
           QuickHull3D_Vector3d res = new QuickHull3D_Vector3d();
...
"Input points appear to be coplanar"); 
           ...
           QuickHull3D_Face[] tris = new QuickHull3D_Face[4];

           if (vtx[3].pnt.dot (nrml) - d0 < 0)
            { tris[0] = QuickHull3D_Face.createTriangle (vtx[0], vtx[1], vtx[2]);
              tris[1] = QuickHull3D_Face.createTriangle (vtx[3], vtx[1], vtx[0]);
              tris[2] = QuickHull3D_Face.createTriangle (vtx[3], vtx[2], vtx[1]);
              tris[3] = QuickHull3D_Face.createTriangle (vtx[3], vtx[0], vtx[2]);
            ...
            { tris[0] = QuickHull3D_Face.createTriangle (vtx[0], vtx[2], vtx[1]);
              tris[1] = QuickHull3D_Face.createTriangle (vtx[3], vtx[0], vtx[1]);
              tris[2] = QuickHull3D_Face.createTriangle (vtx[3], vtx[1], vtx[2]);
              tris[3] = QuickHull3D_Face.createTriangle (vtx[3], vtx[2], vtx[0]);
           ...
           for (int i=0; i<numPoints; i++)
            { QuickHull3D_Vertex v = pointBuffer[i];
         ...
         */
        public QuickHull3D_Point3d[] getVertices()
         {
           QuickHull3D_Point3d[] vtxs = new QuickHull3D_Point3d[numVertices];
        ...
        public int getVertices(double[] coords)
         {
           for (int i=0; i<numVertices; i++)
            { QuickHull3D_Point3d pnt = pointBuffer[vertexPointIndices[i]].pnt;
              coords[i*3+0] = pnt.x;
              coords[i*3+1] = pnt.y;
              coords[i*3+2] = pnt.z;
            }
        ...
        public void print (PrintStream ps, int indexFlags)
         {
           ...
           for (Iterator fi=faces.iterator(); fi.hasNext(); )
            { QuickHull3D_Face face = (QuickHull3D_Face)fi.next();
        ...
        protected void resolveUnclaimedPoints (QuickHull3D_FaceList newFaces)
         {
           QuickHull3D_Vertex vtxNext = unclaimed.first();
           for (QuickHull3D_Vertex vtx=vtxNext; vtx!=null; vtx=vtxNext)
            { vtxNext = vtx.next;
              
              double maxDist = tolerance;
              QuickHull3D_Face maxFace = null;
              for (QuickHull3D_Face newFace=newFaces.first(); newFace != null;
                   newFace=newFace.next)
               { 
                 if (newFace.mark == QuickHull3D_Face.VISIBLE)
        ...
        protected void deleteFacePoints (QuickHull3D_Face face, QuickHull3D_Face absorbingFace)
         {
           QuickHull3D_Vertex faceVtxs = removeAllPointsFromFace (face);
           if (faceVtxs != null)
            { 
              if (absorbingFace == null)
               ...
               { QuickHull3D_Vertex vtxNext = faceVtxs;
                 for (QuickHull3D_Vertex vtx=vtxNext; vtx!=null; vtx=vtxNext)
                  { vtxNext = vtx.next;
        ...
        protected double oppFaceDistance (QuickHull3D_HalfEdge he)
         {
           return he.face.distanceToPlane (he.opposite.face.getCentroid());
         }

        private boolean doAdjacentMerge (QuickHull3D_Face face, int mergeType)
         {
           QuickHull3D_HalfEdge hedge = face.he0;
           ...
           while (hedge != face.he0);
           if (!convex)
            { face.mark = QuickHull3D_Face.NON_CONVEX; 
            }
           return false;
         }

        protected void calculateHorizon (
           QuickHull3D_Point3d eyePnt, QuickHull3D_HalfEdge edge0, QuickHull3D_Face face, Vector horizon)
         {
//         oldFaces.add (face);
           deleteFacePoints (face, null);
           face.mark = QuickHull3D_Face.DELETED;
           if (debug)
            { System.out.println ("  visiting face " + face.getVertexString());
            }
           QuickHull3D_HalfEdge edge;
           if (edge0 == null)
           ...
           do
            { QuickHull3D_Face oppFace = edge.oppositeFace();
              if (oppFace.mark == QuickHull3D_Face.VISIBLE)
        ...
        private QuickHull3D_HalfEdge addAdjoiningFace (
           QuickHull3D_Vertex eyeVtx, QuickHull3D_HalfEdge he)
         { 
           QuickHull3D_Face face = QuickHull3D_Face.createTriangle (
        ...
        protected void addNewFaces (
           QuickHull3D_FaceList newFaces, QuickHull3D_Vertex eyeVtx, Vector horizon)
         { 
           newFaces.clear();

           QuickHull3D_HalfEdge hedgeSidePrev = null;
           QuickHull3D_HalfEdge hedgeSideBegin = null;

           for (Iterator it=horizon.iterator(); it.hasNext(); ) 
            { QuickHull3D_HalfEdge horizonHe = (QuickHull3D_HalfEdge)it.next();
              QuickHull3D_HalfEdge hedgeSide = addAdjoiningFace (eyeVtx, horizonHe);
              if (debug)
               { System.out.println (
        ...
        protected QuickHull3D_Vertex nextPointToAdd()
         {
           if (!claimed.isEmpty())
            { QuickHull3D_Face eyeFace = claimed.first().face;
              QuickHull3D_Vertex eyeVtx = null;
              double maxDist = 0;
              for (QuickHull3D_Vertex vtx=eyeFace.outside;
        ...
        protected void addPointToHull(QuickHull3D_Vertex eyeVtx)
         {
             horizon.clear();
             unclaimed.clear();
             ...
             for (QuickHull3D_Face face = newFaces.first(); face!=null; face=face.next)
              { 
                if (face.mark == QuickHull3D_Face.NON_CONVEX)
                 { face.mark = QuickHull3D_Face.VISIBLE;
        ...
        private void markFaceVertices (QuickHull3D_Face face, int mark)
         {
           QuickHull3D_HalfEdge he0 = face.getFirstEdge();
        ...
        protected boolean checkFaceConvexity (
           QuickHull3D_Face face, double tol, PrintStream ps)
         {
           double dist;
           QuickHull3D_HalfEdge he = face.he0;
           do
