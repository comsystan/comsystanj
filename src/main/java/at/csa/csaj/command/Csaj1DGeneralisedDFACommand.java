 ...
 * File: Csaj1DGeneralisedDFA.java
 * 
 * $Id$
 * $HeadURL$
 * 
 ...
 *
 */
public class Csaj1DGeneralisedDFACommand<T extends RealType<T>> extends ContextCommand implements Previewable {
...
//	private final String labelSpace = SPACE_LABEL;
	...
	@Parameter(label = "Regression Start", description = "Minimum x value of linear regression", style = NumberWidget.SPINNER_STYLE, min = "4", max = "9999999999999999999", stepSize = "1",
			   persist = false, //restore previous value default = true
			   initializer = "initialNumRegStart", callback = "callbackNumRegStart")
	private int spinnerInteger_NumRegStart = 1;

	@Parameter(label = "Regression End", description = "Maximum x value of linear regression", style = NumberWidget.SPINNER_STYLE, min = "5", max = "9999999999999999999", stepSize = "1",
			   persist = false, //restore previous value default = true
...
//			}
	...
	private void writeToTable(int numRow, int sequenceNumber, Container_ProcessMethod containerPM) {
		...
		tableOut.set(8, row, "("+spinnerInteger_NumRegStart+")" + containerPM.item2_Values[0]); //(numRegStart)epsRegStart //numRegStart
		tableOut.set(9, row, "("+spinnerInteger_NumRegEnd  +")" + containerPM.item2_Values[1]); //(numRegEnd)epsRegEnd     //numRegEnd	
		tableColLast = 9;
			...
			if (surrType.equals("No surrogates")) {
				...
				if (sequence1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
					...
					if (optShowPlot) {
						...
						//showPlot(genDfa.getLnDataX(), genDfa.getLnDataY(), preName, col, numRegStart, numRegEnd);
