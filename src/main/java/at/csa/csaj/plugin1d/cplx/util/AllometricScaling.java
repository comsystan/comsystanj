...
public class AllometricScaling {
	 ...
	 */
	public double[] calcLogRegression(double[][] asData, int numRegStart, int numRegEnd){
		double[] regressionParams;
		
		if (numRegEnd > asData[0].length){
			numRegEnd = asData[0].length;
			logService.info(this.getClass().getName() + " AllometricScaling: Regression max > 1/4 sequence size is not allowed");
			JOptionPane.showMessageDialog(null, "AllometricScaling: Regression max > 1/4 sequence size is not allowed!", "Warning", JOptionPane.WARNING_MESSAGE);
			regressionParams = new double[5];
			for (int p = 0; p < regressionParams.length; p++) regressionParams[p] = Double.NaN;
			return regressionParams;
		}
		
		eps     = new double[asData[0].length];
		...
		for (int i = 0; i < asData[0].length; i++){
			...
			if (Double.isNaN(lnX)){
				...
				JOptionPane.showMessageDialog(null, "AllometricScaling: NaN value detected, maybe a mean is negative, which is not allowed for allometric scaling", "Warning", JOptionPane.WARNING_MESSAGE); 
			}
			if (Double.isNaN(lnY)){
				NaNdetected = true;
				logService.info(this.getClass().getName() + " AllometricScaling: NaN value detected, maybe a mean is negative, which is not allowed for allometric scaling");
