...
import at.csa.csaj.plugin1d.cplx.util.RescaledAutoDensity;
import at.csa.csaj.plugin1d.misc.Csaj1DOpenerCmd;

/**
 * A {@link ContextCommand} plugin computing the <rescaled auto-density (RAD)</a>
 * of a sequence.
 * 
 * Rescaled auto-density (RAD) noise-insensitive measure of the distance to criticality.
...
@Plugin(type = ContextCommand.class, 
		headless = true,
		label = "Rescaled auto-density RAD",
		initializer = "initialPluginLaunch",
		iconPath = "/icons/comsystan-logo-grey46-16x16.png", //Menu entry icon
		menu = {}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 

public class Csaj1DRADCmd<T extends RealType<T>> extends ContextCommand implements Previewable {

	private static final String PLUGIN_LABEL            = "<html><b>Rescaled auto-density RAD</b></html>";
	private static final String SPACE_LABEL             = "";
	//private static final String REGRESSION_LABEL      = "<html><b>Regression parameters</b></html>";
	private static final String LAG_LABEL               = "<html><b>LAG parameters</b></html>";
	...
	public static final String TABLE_OUT_NAME = "Table - RAD";
...
//    private String choiceRadioButt_Algorithm;
	...
	//-----------------------------------------------------------------------------------------------------
	@Parameter(label = " ", visibility = ItemVisibility.MESSAGE, persist = false)
	private final String labelLagOptions = LAG_LABEL;

	
	@Parameter(label = "Tau",
			   description = "Time lag/delay",
			   style = NumberWidget.SPINNER_STYLE, 
			   min = "1",
			   max = "9999999999999999999",
			   stepSize = "1",
...
//	}
	...
	*/
	protected void startWorkflowForSingleColumn() {
		...
		dlgProgress = new CsajDialog_WaitingWithProgressBar("Computing Rescaled auto-density, please wait... Open console window for further info.",
	...
	*/
	protected void startWorkflowForAllColumns() {
		...
		dlgProgress = new CsajDialog_WaitingWithProgressBar("Computing Rescaled auto-densities, please wait... Open console window for further info.",
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SequenceRange.equals("Entire sequence")){
			tableOut.add(new DoubleColumn("RAD"));	
			tableOut.add(new DoubleColumn("cRAD"));
			
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				//do nothing	
			} else { //Surrogates
				tableOut.add(new DoubleColumn("RAD_Surr"));   //Mean surrogate value	
				tableOut.add(new DoubleColumn("sRAD_Surr"));  //Mean surrogate value
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("RAD_Surr-#"+(s+1))); 
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("cRAD_Surr-#"+(s+1)));  
			}	
		} 
		else if (choiceRadioButt_SequenceRange.equals("Subsequent boxes")){
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableOut.add(new DoubleColumn("RAD-#" + n));	
			}
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableOut.add(new DoubleColumn("cRAD-#" + n));	
			}
		}
...
//			}
	...
	private void processSingleInputColumn (int c) {
		...
		// 0 RAD, 1 cRAD
		if (containerPM != null) {
			logService.info(this.getClass().getName() + "  RAD: " + containerPM.item1_Values[0]);
			logService.info(this.getClass().getName() + " cRAD: " + containerPM.item1_Values[1]);
		}
	...
	*/
	private CsajContainer_ProcessMethod process(DefaultGenericTable dgt, int col) { //  c column number
		...
		double[] resultValues = new double[2]; // // 0 RAD, 1 cRAD
		...
		RescaledAutoDensity rad;
		double[] radValues = null;

		//"Entire sequence", "Subsequent boxes", "Gliding box" 
		//********************************************************************************************************
		if (sequenceRange.equals("Entire sequence")){	
			if (surrType.equals("No surrogates")) {
				resultValues = new double[2]; // RAD cRAD
			} else {
				resultValues = new double[2+2+2*numSurrogates]; // RAD_Surr, cRAD_Surr, RAD_Surr1, RAD_Surr2, RAD_Surr3,...Surr2...Surr3...
			}
			for (int r = 0; r < resultValues.length; r++) resultValues[r] = Double.NaN;
			...
			if (sequence1D.length > (tau * 2)) { // only data series which are large enough
				...
				rad = new RescaledAutoDensity();
				radValues = rad.calcDensities(sequence1D, tau); //RAD cRAD	
				resultValues[0] = radValues[0]; //RAD
				resultValues[1] = radValues[1]; //cRAD
				...
				if (!surrType.equals("No surrogates")) { //Add surrogate analysis
					...
					for (int s = 0; s < numSurrogates; s++) {
						...
						rad = new RescaledAutoDensity();
						radValues = rad.calcDensities(surrSequence1D, tau); //RAD cRAD
				
						resultValues[lastMainResultsIndex + 4 +                 + s] = radValues[0];
						resultValues[lastMainResultsIndex + 4 +   numSurrogates + s] = radValues[1];
	
						sumRAD  += radValues[0];
						sumCRAD += radValues[1];

					}
					resultValues[lastMainResultsIndex + 1] = sumRAD/numSurrogates;
					resultValues[lastMainResultsIndex + 2] = sumCRAD/numSurrogates;
				}	
			} 
		//********************************************************************************************************	
		} else if (sequenceRange.equals("Subsequent boxes")){
			resultValues = new double[(int) (3*numSubsequentBoxes)]; // RAD cRAD  == 2* number of boxes		
			for (int r = 0; r<resultValues.length; r++) resultValues[r] = Double.NaN;
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				rad = new RescaledAutoDensity();
				radValues = rad.calcDensities(subSequence1D, tau); //RAD cRAD	
				
				resultValues[i]                               = radValues[0]; //RAD
				resultValues[(int)(i +   numSubsequentBoxes)] = radValues[1]; //cRAD		
				//***********************************************************************
			}	
		//********************************************************************************************************			
		} else if (sequenceRange.equals("Gliding box")){
			resultValues = new double[(int) (3*numGlidingBoxes)]; // RAD cRAD == 2 * number of boxes	
			for (int r = 0; r<resultValues.length; r++) resultValues[r] = Double.NaN;
			subSequence1D = new double[(int) numBoxLength];
			...
			for (int i = 0; i < actualNumGlidingBoxes; i++) {
				...
				resultValues[i]                            = radValues[0]; //RAD
				resultValues[(int)(i +   numGlidingBoxes)] = radValues[1]; //cRAD		
				//***********************************************************************
			}
		}
