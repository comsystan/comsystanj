...
/**
 * A {@link InteractiveCommand} plugin computing <the Lyapunov exponent</a>
* of a sequence.
 */
@Plugin(type = InteractiveCommand.class, 
	...
	@Menu(label = "Lyapunov exponent ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 
/**
 ...
 *
 */
public class Csaj1DLyapunov<T extends RealType<T>> extends InteractiveCommand implements Previewable {

	private static final String PLUGIN_LABEL            = "<html><b>Lyapunov exponent</b></html>";
	...
	public static final String TABLE_OUT_NAME = "Table - Lyapunov exponent";
	...
	@Parameter(label = "(Rosen/Kantz)Tau",
	...
	@Parameter(label = "(Rosen/Kantz)Sampl. frequ.(Hz)",
			   description = "Sampling frequency. If not known set it to 1",
			   style = NumberWidget.SPINNER_STYLE, 
			   min = "1",
			   max = "9999999999999999999",
			   stepSize = "1",
			   persist = true, // restore  previous value  default  =  true
			   initializer = "initialSampFrequ",
			   callback = "callbackSampFrequ")
	private int spinnerInteger_SampFrequ;
	
    @Parameter(label = "(Kantz)Point pairs #",
...
//		}
	...
	protected void initialSampFrequ() {
		spinnerInteger_SampFrequ = 1;
	}
	...
	protected void callbackKMax() {
		...
		if (spinnerInteger_NumRegStart >= spinnerInteger_NumRegEnd - 1) {
			spinnerInteger_NumRegStart  = spinnerInteger_NumRegEnd - 1;
		}
	...
	protected void callbackNumRegEnd() {
		if (spinnerInteger_NumRegEnd <= spinnerInteger_NumRegStart + 1) {
			spinnerInteger_NumRegEnd  = spinnerInteger_NumRegStart + 1;
		}
		if (spinnerInteger_NumRegEnd > spinnerInteger_KMax) {
			spinnerInteger_NumRegEnd = spinnerInteger_KMax;
		}
	...
	/** Executed whenever the {@link #spinnerInteger_SampFrequ} parameter changes. */
	protected void callbackSampFrequ() {
	...
	*/
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SequenceRange.equals("Entire sequence")){
			tableOut.add(new DoubleColumn("λ"));	
			...
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				...
				tableOut.add(new DoubleColumn("λ_Surr")); //Mean surrogate value	
				tableOut.add(new DoubleColumn("R2_Surr")); //Mean surrogate value
				tableOut.add(new DoubleColumn("StdErr_Surr")); //Mean surrogate value
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("λ_Surr-#"+(s+1))); 
	...
	private CsajContainer_ProcessMethod process(DefaultGenericTable dgt, int col) { //  c column number
		...
		if (sequenceRange.equals("Entire sequence")){	
			if (surrType.equals("No surrogates")) {
				resultValues = new double[3]; // Lya, R2, StdErr	
			} else {
			...
			if (sequence1D.length > (numKMax * 2)) { // only data series which are large enough
				...
				if (algorithm.equals("Rosenstein")) {
					...
					epsRegStartEnd[1] = lya.getEps()[numRegEnd-1];   //epsRegEnd
					...
					if (optShowPlot) {
						String preName = sequenceColumn.getHeader();
						//showPlot(lya.getLnDataX(), lya.getLnDataY(), preName, col, numRegStart, numRegEnd);
						showPlots(lya.getEps(), lya.getDataYs(), preName, col, numRegStart, numRegEnd);
					}
				}
		...
		//********************************************************************************************************	
			...
			resultValues = new double[(int) (2*numSubsequentBoxes)]; // Lya R2 == two* number of boxes		
			for (int r = 0; r<resultValues.length; r++) resultValues[r] = Double.NaN;
		...
		//********************************************************************************************************			
			...
			resultValues = new double[(int) (2*numGlidingBoxes)]; // Lya R2 == two * number of boxes	
			for (int r = 0; r<resultValues.length; r++) resultValues[r] = Double.NaN;
			subSequence1D = new double[(int) numBoxLength];
