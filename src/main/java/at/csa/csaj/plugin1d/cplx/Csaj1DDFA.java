...
/**
 * A {@link ContextCommand} plugin computing <DFA</a>
* of a sequence.
 */
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Detrended fluctuation analysis ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 
//public class Csaj1DDFA<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
...
//		}
		...
		if (sequenceRange.equals("Entire sequence")){	
			...
			if (sequence1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
				...
				epsRegStartEnd[1] = dfa.getEps()[numRegEnd-1];   //epsRegEnd
