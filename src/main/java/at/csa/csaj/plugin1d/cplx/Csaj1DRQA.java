...
/**
 * A {@link ContextCommand} plugin computing <Recurrence quantification analysis</a>
* of a sequence.
 */
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Recurrence quantification analysis ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 
//public class Csaj1DRQA<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
...
//			}
	...
	private void writeToTable(int numRow, int sequenceNumber, Container_ProcessMethod containerPM) {
		...
		//tableOut.set( 8, row, "("+spinnerInteger_NumRegStart+")" + containerPM.result2_Values[0]); //(NumRegStart)epsRegStart    //NumRegStart
		//tableOut.set( 9, row, "("+spinnerInteger_NumRegEnd+")" + containerPM.result2_Values[1]); //(NumRegEnd)epsRegEnd    //NumRegEnd	
	...
	*/
	private Container_ProcessMethod process(DefaultGenericTable dgt, int col) { //  c column number
		...
		Leshao_RecurrentMatrix rm;
		
		//Copy to LimitedQueue
		//This may be skipped for a future release
		Leshao_LimitedQueue<Float> timeSeries = new Leshao_LimitedQueue<Float>(sequence1D.length);//Initiate data
			...
			if (sequence1D.length > (tau * 2)) { // only data series which are large enough
				...
				if (!surrType.equals("No surrogates")) { //Add surrogate analysis
					...
					for (int s = 0; s < numSurrogates; s++) {
						...
						if (algorithm.equals("RQA")) {
							timeSeries = new Leshao_LimitedQueue<Float>(surrSequence1D.length);//Initiate data
		...
		//********************************************************************************************************	
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				timeSeries = new Leshao_LimitedQueue<Float>(subSequence1D.length);//Initiate data
