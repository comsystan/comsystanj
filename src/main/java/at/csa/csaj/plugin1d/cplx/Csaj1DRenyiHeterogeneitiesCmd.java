 ...
 * File: Csaj1DRenyiHeterogeneitiesCmd.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
/**
 * A {@link ContextCommand} plugin computing <Renyi heterogeneities</a>
 * of a sequence.
 * <li> According to
...
@Plugin(type = ContextCommand.class, 
		headless = true,
		label = "Renyi heterogeneities",
		initializer = "initialPluginLaunch",
		iconPath = "/icons/comsystan-logo-grey46-16x16.png", //Menu entry icon
		menu = {}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 

public class Csaj1DRenyiHeterogeneitiesCmd<T extends RealType<T>> extends ContextCommand implements Previewable {

	private static final String PLUGIN_LABEL            = "<html><b>Renyi heterogeneities</b></html>";
	...
	public static final String TABLE_OUT_NAME = "Table - Renyi heterogeneities";
...
//		stepQ     = 1;
	...
	*/
	protected void startWorkflowForSingleColumn() {
		...
		dlgProgress = new CsajDialog_WaitingWithProgressBar("Computing Renyi heterogeneities, please wait... Open console window for further info.",
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SequenceRange.equals("Entire sequence")){
			...
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				...
				tableOut.add(new DoubleColumn("RenyiHet_q"   + minQ)); 
				tableOut.add(new DoubleColumn("RenyiHet_q"+minQ+"_Surr"));  //Mean surrogate value	
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("RenyiHet_q"+minQ+"_Surr#"+(s+1))); 
			}
		} 
		else if (choiceRadioButt_SequenceRange.equals("Subsequent boxes")){
			String entropyHeader = "RenyiHet_q"+minQ;			
...
//			}
	...
	private void processSingleInputColumn (int c) {
		...
		// 0 
		logService.info(this.getClass().getName() + " Renyi heterogeneity: " + containerPM.item1_Values[0]);
	...
	*/
	private CsajContainer_ProcessMethod process(DefaultGenericTable dgt, int col) { //  c column number
		...
		double hetValue = Float.NaN;
		CsajAlgorithm_RenyiHeterogeneities rh;
		...
		if (sequenceRange.equals("Entire sequence")){	
			...
			if (surrType.equals("No surrogates")) {		
				resultValues = new double[numOfHets]; // 		
				for (int r = 0; r < resultValues.length; r++) resultValues[r] = Float.NaN;
				...
				rh = new CsajAlgorithm_RenyiHeterogeneities(probabilities);
				...
				if (optShowRenyiPlot) {	
					...
					CsajPlot_SequenceFrame dimGenPlot = DisplaySinglePlotXY(qList, entList, isLineVisible, "Renyi heterogeneities", 
							preName + tableInName, axisNameX, axisNameY, "");
					genRenyiPlotList.add(dimGenPlot);
				}		
				
			} else {
				resultValues = new double[1+1+1*numSurrogates]; // Het,  Het_SurrMean, Het_Surr#1, Entropy_Surr#2......
				
				probabilities = compProbabilities(sequence1D, lag, probType);		
				rh = new CsajAlgorithm_RenyiHeterogeneities(probabilities);
				
				hetValue = (rh.compRenyiHets  (minQ, maxQ, numQ, skipZeroBin))[0];

				resultValues[0] = hetValue;
				...
				for (int s = 0; s < numSurrogates; s++) {
					...
					hetValue = (rh.compRenyiHets  (minQ, maxQ, numQ, skipZeroBin))[0]; //value for min

					resultValues[lastMainResultsIndex + 2 + s] = hetValue;
					sumHets += hetValue;
				}
				resultValues[lastMainResultsIndex + 1] = sumHets/numSurrogates;
			}
