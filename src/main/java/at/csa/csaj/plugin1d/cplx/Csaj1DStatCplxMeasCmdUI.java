 ...
 * File: Csaj1DStatCplxMeasCmdUI.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
@Plugin(type = ContextCommand.class,
		...
		@Menu(label = "Statistical complexity measures ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 

public class Csaj1DStatCplxMeasCmdUI extends ContextCommand implements Previewable{
	...
	private Csaj1DStatCplxMeasDialog dialog = null;
	...
	public void run() {	
		SwingUtilities.invokeLater(() -> {
			if (dialog == null) {
				dialog = new Csaj1DStatCplxMeasDialog(context(), defaultTableDisplay);
			}
