 ...
 * File: Csaj1DSymbolicAggregationCmdUI.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
@Plugin(type = ContextCommand.class,
		...
		@Menu(label = "Symbolic aggregation(New dialog) ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 

public class Csaj1DSymbolicAggregationCmdUI extends ContextCommand implements Previewable{
	...
	private Csaj1DSymbolicAggregationDialog dialog = null;
	...
	public void run() {	
		SwingUtilities.invokeLater(() -> {
			if (dialog == null) {
				dialog = new Csaj1DSymbolicAggregationDialog(context(), defaultTableDisplay);
			}
