 ...
 * see Wang et al., Fractal Analysis on Artificial Profiles and Electroencephalography Sequences by Roughness Scaling Extraction Algorithm
 ...
 * Li et al. state that RSE dimension is smaller than 1 for non-fractal sequences (constructed with -2 < D_W-M  < 1 ), see Figure 2
...
//							tableForPlot.add(new GenericColumn("Sequence"));
//							tableForPlot.add(new GenericColumn("Fitted sequence"));
...
//							String sequenceTitle = "Sequences(s)";
//							String xLabel = "#";
//							String yLabel = "Value";
...
//							}
//							SequencePlotFrame pdf = new SequencePlotFrame(tableForPlot, cols, isLineVisible, "Sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
...
//						} //show flattened subSequences				
	 ...
	 */
	public double[] calcRegression(double[] Rq, int numRegStart, int numRegEnd) {
		
		eps     = new double[Rq.length];
		...
		for (int i = 0; i < Rq.length; i++) {
			eps[i] = i + 1;
			if (Rq[i] == 0)
				Rq[i] = Double.MIN_VALUE;
		}
	
		for (int i = 0; i < Rq.length; i++) {
			double lnX = Math.log(eps[i]); //lengths
