 ...
 * File: Csaj1DFracDimHiguchi1D.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
import at.csa.csaj.plugin1d.frac.util.Higuchi;
import at.csa.csaj.plugin1d.misc.Csaj1DOpener;

/**
 * A {@link ContextCommand} plugin computing <the Higuchi dimension</a>
* of a sequence.
 */
...
//public class Csaj1DFracDimHiguchi1D<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj1DFracDimHiguchi1D<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//			}
	...
	private void processSingleInputColumn (int c) {
		...
		// 0 Dh, 1 R2, 2 StdErr
		logService.info(this.getClass().getName() + " Higuchi dimension: " + containerPM.item1_Values[0]);
...
//		}
		...
		if (sequenceRange.equals("Entire sequence")){	
			...
			if (sequence1D.length > (numKMax * 2)) { // only data series which are large enough
				...
				epsRegStartEnd[1] = hig.getEps()[numRegEnd-1];   //epsRegEnd
	...
	private void showPlot(double[] lnDataX, double[] lnDataY, String preName, int col, int numRegStart, int numRegEnd) {
		...
		Plot_RegressionFrame doubleLogPlot = DisplayRegressionPlotXY(lnDataX, lnDataY, isLineVisible,
				"Double log plot - Higuchi dimension", preName + "-" + tableInName, "ln(k)", "ln(L)", "", numRegStart, numRegEnd);
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Csaj1DFracDimHiguchi1D.class, true);
	}
}
