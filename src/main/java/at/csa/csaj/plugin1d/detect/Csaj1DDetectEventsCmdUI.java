 ...
 * File: Csaj1DDetectEventsCmdUI.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
@Plugin(type = ContextCommand.class,
		...
		@Menu(label = "Event detection(New dialog) ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 

public class Csaj1DDetectEventsCmdUI extends ContextCommand implements Previewable{
	...
	private Csaj1DDetectEventsDialog dialog = null;
	...
	public void run() {	
		SwingUtilities.invokeLater(() -> {
			if (dialog == null) {
				dialog = new Csaj1DDetectEventsDialog(context(), defaultTableDisplay);
			}
