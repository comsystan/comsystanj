...
@Plugin(type = ContextCommand.class,
	...
	@Menu(label = "Filter ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 
//public class Csaj1DFilter<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
	...
	private void processSingleInputColumn (int s) {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			if (containerPM != null) { 
				...
				Plot_SequenceFrame pdf = new Plot_SequenceFrame(tableOut, cols, isLineVisible, "Filtered sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
