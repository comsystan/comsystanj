 ...
 * File: Csaj1DResamplingCmdUI.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
@Plugin(type = ContextCommand.class,
		...
		@Menu(label = "Resampling(New dialog) ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 

public class Csaj1DResamplingCmdUI extends ContextCommand implements Previewable{
	...
	private Csaj1DResamplingDialog dialog = null;
	...
	public void run() {	
		SwingUtilities.invokeLater(() -> {
			if (dialog == null) {
				dialog = new Csaj1DResamplingDialog(context(), defaultTableDisplay);
			}
