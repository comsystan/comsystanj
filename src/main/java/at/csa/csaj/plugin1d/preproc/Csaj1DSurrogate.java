...
/**
 * A {@link ContextCommand} plugin computing <surrogates</a>
 * of a sequence.
 */
@Plugin(type = ContextCommand.class,
	...
	@Menu(label = "Surrogates ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 
//public class Csaj1DSurrogate<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
	...
	private void processSingleInputColumn (int s) {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			if (containerPM != null) { 
				...
				Plot_SequenceFrame pdf = new Plot_SequenceFrame(tableOut, cols, isLineVisible, "Surrogate sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
...
//		}
		...
		return new Container_ProcessMethod(sequence1D);
		// 
		// Output
