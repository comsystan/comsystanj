...
package at.csa.csaj.plugin1d.preproc.surrogate;
...
@Plugin(type = ContextCommand.class,
	...
	@Menu(label = "Surrogates ", weight = 70)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SequenceSurrogate<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj1DSurrogate<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
	...
	private void deleteExistingDisplays() {
		...
		if (optDeleteExistingImgs) {
			...
			for (int i = listFrames.length -1 ; i >= 0; i--) { //Reverse order, otherwise focus is not given free from the last image
				...
				if (frame.getTitle().contains("Surrogate sequence(s)")) {
	...
	private void processSingleInputColumn (int s) {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			if (resultValues != null) { 
				...
				String sequenceTitle = "Surrogate - " + this.choiceRadioButt_SurrogateType;
				String xLabel = "#";
				String yLabel = "Value";
				...
				SequencePlotFrame pdf = new SequencePlotFrame(tableOut, cols, isLineVisible, "Surrogate sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
...
//		}
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Csaj1DSurrogate.class, true);
	}
}
