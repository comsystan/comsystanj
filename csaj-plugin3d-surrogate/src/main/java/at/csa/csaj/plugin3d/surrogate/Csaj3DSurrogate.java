/*-
 * #%L
 * Project: ImageJ2 plugin for constructing a 3D surrogate image volume
 * File: Csaj3DSurrogate.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
package at.csa.csaj.plugin3d.surrogate;
...
import at.csa.csaj.commons.algorithms.Surrogate3D;
...
@Menu(label = "3D Surrogate", weight = 18)})
//public class Csaj3DSurrogate<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj3DSurrogate<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//			datasetPreview.setColorTable(ColorTables.BLUE,  2);
	...
	*/
	protected void startWorkflowForSingleVolume() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("3D surrogate, please wait... Open console window for further info.",
...
//		}
	...
	Surrogate3D surrogate3D = new Surrogate3D();
	...
	public static void main(final String... args) throws Exception {
		...
		// ij.command().run(Csaj3DSurrogate.class,
		// true).get().getOutput("image");
		ij.command().run(Csaj3DSurrogate.class, true);
	}
}
