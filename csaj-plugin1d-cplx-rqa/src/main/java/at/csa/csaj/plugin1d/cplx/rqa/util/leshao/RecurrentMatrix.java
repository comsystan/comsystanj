...
public class RecurrentMatrix{
  private LimitedQueue<Float> ts1, ts2;
  int dim, lag;
  ...
  public RecurrentMatrix(LimitedQueue<Float> ts1,LimitedQueue<Float> ts2, int dim,int lag, float eps, boolean takeDiagonal, String norm){
	this.ts1 = ts1;
	this.ts2 = ts2;
	this.dim = dim;
	this.lag = lag;
  ...
  public RecurrentMatrix(LimitedQueue<Float> ts, int dim, int lag, float radius, boolean takeDiagonal, String norm){
  ...
  public void calcEuclideanDistanceMatrix(){
	  LimitedQueue<float[]> reconstruct1 = PhaseSpaceReconstruct.go(ts1, dim, lag);
