/*-
 * #%L
 * Project: ImageJ signal plugin for computing fractal dimension with Sevcik algorithm.
 * File: SignalFractalDimensionSevcik.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
package at.csa.csaj.sig.frac.dim.sevcik;
...
import at.csa.csaj.sig.frac.dim.sevcik.util.Sevcik;
...
public class SignalFractalDimensionSevcik<T extends RealType<T>> extends InteractiveCommand implements Command, Previewable { // non blocking  GUI
//public class SignalFractalDimensionSevcik<T extends RealType<T>> implements Command {	//modal GUI
...
//	private final String labelBackgroundOptions = BACKGROUNDOPTIONS_LABEL;
	...
	protected void callbackProcessSingleColumn() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing Sevcik dimensions, please wait...<br>Open console window for further info.</html>");
		dlgProgress = new WaitingDialogWithProgressBar("Computing Sevcik dimensions, please wait... Open console window for further info.",
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SignalRange.equals("Entire signal")){
			tableResult.add(new DoubleColumn("Ds"));	
			
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				//do nothing	
			} else { //Surrogates
				tableResult.add(new DoubleColumn("Ds_Surr")); //Mean surrogate value	
			
				for (int s = 0; s < numSurrogates; s++) tableResult.add(new DoubleColumn("DsSurr-#"+(s+1))); 
			}	
		} 
		else if (choiceRadioButt_SignalRange.equals("Subsequent boxes")){
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableResult.add(new DoubleColumn("Ds-#" + n));	
			}
		}
...
//		}
		...
		if (signalRange.equals("Entire signal")){	
			...
			sevcik = new Sevcik();
			Ds = sevcik.calcDimension(signal1D);
			...
			if (!surrType.equals("No surrogates")) { //Add surrogate analysis
				...
				for (int s = 0; s < numSurrogates; s++) {
					...
					sevcik = new Sevcik();
					Ds = sevcik.calcDimension(surrSignal1D);
		...
		//********************************************************************************************************	
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				sevcik = new Sevcik();
				Ds = sevcik.calcDimension(subSignal1D);
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(SignalFractalDimensionSevcik.class, true);
	}
}
