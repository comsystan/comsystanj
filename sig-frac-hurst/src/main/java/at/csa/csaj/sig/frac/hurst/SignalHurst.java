 ...
 * #L%
 */
package at.csa.csaj.sig.frac.hurst;
...
import at.csa.csaj.sig.frac.hurst.util.BetaDispH;
import at.csa.csaj.sig.frac.hurst.util.BetaPSD;
import at.csa.csaj.sig.frac.hurst.util.BetaSWVH;
 ...
 * of a  signal.
 */
@Plugin(type = InteractiveCommand.class, headless = true, menuPath = "Plugins>ComsystanJ>Signal>Fractal - Hurst")
public class SignalHurst<T extends RealType<T>> extends InteractiveCommand implements Command, Previewable { // non blocking  GUI
//public class SignalHurst<T extends RealType<T>> implements Command {	//modal GUI
	...
	private static double[] signalNew;
	private static double[] signalSSC;
...
//		}
	...
	protected void callbackAnalysisType() {
		logService.info(this.getClass().getName() + " Analysis type set to " + choiceRadioButt_AnalysisType);
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_AnalysisType.equals("Entire signal")){
			...
			tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"_Beta"));	
			tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"_H"));	
			tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"_R2"));	
			tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"_StdErr"));	
			
			tableResult.add(new DoubleColumn("SSC_Beta"));	
			tableResult.add(new DoubleColumn("SSC_R2"));	
			tableResult.add(new DoubleColumn("SSC_StdErr"));	
			
			tableResult.add(new DoubleColumn("Disp_Beta"));	
			tableResult.add(new DoubleColumn("Disp_H"));		
			tableResult.add(new DoubleColumn("Disp_R2"));
			tableResult.add(new DoubleColumn("Disp_StdErr"));
			
			tableResult.add(new DoubleColumn(choiceRadioButt_SWVType+"_Beta"));	
			tableResult.add(new DoubleColumn(choiceRadioButt_SWVType+"_H"));	
			tableResult.add(new DoubleColumn(choiceRadioButt_SWVType+"_R2"));	
			tableResult.add(new DoubleColumn(choiceRadioButt_SWVType+"_StdErr"));	
			
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				//do nothing	
			} else { //Surrogates
				tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"_Beta_Surr")); //Mean surrogate value	
				tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"R2_Surr"));    //Mean surrogate value
				for (int s = 0; s < numSurrogates; s++) tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"_Beta_Surr-#"+(s+1))); 
			...
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableResult.add(new DoubleColumn(choiceRadioButt_PSDType+"_Beta-#" + n));	
			}
			for (int n = 1; n <= numSubsequentBoxes; n++) {
...
//			}
	...
	private void writeToTable(int rowNumber, int signalNumber, double[] resultValues) {
		...
		tableResult.set(2, row, choiceRadioButt_AnalysisType);  //Analysis type
		...
		tableResult.set(7, row, regMin); //may be changed by the algorithm, particularly by lowPSD
		tableResult.set(8, row, regMax); //may be changed by the algorithm, particularly by lowPSD
	
		tableColLast = 8;
		
		//"Entire signal", "Subsequent boxes", "Gliding box" 
		if (choiceRadioButt_AnalysisType.equals("Entire signal")){
			tableResult.set(tableColLast + 1, row, signalType); //Signal type fGn or fBm or ?
			int numParameters = resultValues.length;
			tableColStart = tableColLast + 1 + 1;
...
//		}
		...
		if (analysisType.equals("Entire signal")){	
			...
			if (psdType.equals("PSD")){	
				//check endpoint of regression, because power spectrum is only half the size of the signal
				int poweSpecLength = signal1D.length/2;
				if (regMax > poweSpecLength){
					regMax = poweSpecLength;
					this.spinnerInteger_RegMax = poweSpecLength;
					this.callbackRegMax();
				}		
				resultPSD = betaPSD.computeRegression(signal1D, regMin, regMax);		
			}
			else if (psdType.equals("lowPSDwe")){
			
				//apply parabolic windowing	
				signalNew = this.calcParabolicWindowing(signal1D); 
				//apply bridge dentrending
				signalNew = this.calcBridgeDetrending(signalNew);
				...
				resultPSD = betaPSD.computeRegression(signalNew, regMin, regMax);	
			}
			...
			if (optShowPlot) {	
				String preName = "Power Spectral Densitiy " +signalColumn.getHeader();
			...
			if (psd_Beta <= -1.2){
				logService.info(this.getClass().getName() + " Signal type is wether fGn nor fBm");
				signalIsfGn     = false;
				signalIsfBm     = false;
			...
			if ((psd_Beta <= 0.38) && (psd_Beta >-1.2)){ //fGn
				logService.info(this.getClass().getName() + " fGn signal detected");
				signalIsfGn     = true;
				signalIsfBm     = false;	
			...
			if ((psd_Beta >= 1.04) && (psd_Beta < 3.2)){ //fBm
				logService.info(this.getClass().getName() + " fBm signal detected");
				signalIsfGn     = false;
				signalIsfBm     = true;	
				signalIsUnknown = false;
				signalType = "fBm";
			...
			if ((psd_Beta > 0.38) && (psd_Beta < 1.04)){ //signal between fGn and fBm
				logService.info(this.getClass().getName() + " Signal type could not be detected, starting SSC method");
				//signal summation conversion (fGn to fBm, fBm to summed fBm)
				signalSSC = new double[signal1D.length];
				signalSSC[0] = signal1D[0];		
				for (int i = 1; i < signal1D.length; i++){
					signalSSC[i] = signalSSC[i-1] + signal1D[i];
				}
				
				betaSWVH = new BetaSWVH();
				resultSSC= betaSWVH.computeRegression(signalSSC);
				ssc_Beta    = -resultSSC[0];
				ssc_r2      = resultSSC[1];
				ssc_stdErr  = resultSSC[2];
				
				if (optShowPlot) {	
					String preName = "Scaled window variance for H " +signalColumn.getHeader();
					showPlot(betaSWVH.getLnDataX(), betaSWVH.getLnDataY(), preName, col, "ln(winSize/winSize0)", "ln(SD/SD0)", 1, betaSWVH.getLnDataX().length);
				}	

				if (ssc_Beta <= 0.6){ //fGn
					logService.info(this.getClass().getName() + " fGn signal after SSC detected");
					signalIsfGn     = true;
					signalIsfBm     = false;	
					signalIsUnknown = false;
					signalType      = "fGn";
				...
				if (ssc_Beta >= 1.0){ //fBm
					logService.info(this.getClass().getName() + " fBm signal after SSC detected");
					signalIsfGn     = false;
					signalIsfBm     = true;	
					signalIsUnknown = false;
					signalType      = "fBm";
				...
				if ((ssc_Beta > 0.6) && (ssc_Beta < 1.0)){ //not clear
					logService.info(this.getClass().getName() + " Signal type could not be detected, even using SSC");
					signalIsfGn     = false;
					signalIsfBm     = false;	
					signalIsUnknown = true;
					signalType      = "?";
			...
			if (signalIsfGn){ //fGn
				...
				resultDispH = betaDispH.computeRegression(signal1D);
				...
				if (optShowPlot) {	
					String preName = "Dispersional plot for H fGn " +signalColumn.getHeader();
			...
			if (signalIsfBm){ //fBm
				...
				if (swvType.equals("SWV")){
					resultSWVH	= betaSWVH.computeRegression(signal1D);
				}
				if (swvType.equals("bdSWV")){
					double[] signalNew = this.calcBridgeDetrending(signal1D); 
					resultSWVH	= betaSWVH.computeRegression(signalNew);
				}
			
				if (optShowPlot) {	
					String preName = "Scaled window variance for H fBm " +signalColumn.getHeader();
					showPlot(betaSWVH.getLnDataX(), betaSWVH.getLnDataY(), preName, col, "ln(winSize/winSize0)", "ln(SD/SD0)", 1, betaSWVH.getLnDataX().length);
				}	
			...
			if (!surrType.equals("No surrogates")) { //Add surrogate analysis
				...
				for (int s = 0; s < numSurrogates; s++) {
					...
					if (this.choiceRadioButt_HurstType.equals("PSD_Beta")) {
						poweSpecLength = surrSignal1D.length/2;	
						if (psdType.equals("PSD")){			
							resultPSD = betaPSD.computeRegression(surrSignal1D, 1 , poweSpecLength);		
						}
						else if (psdType.equals("lowPSDwe")){
						
							//apply parabolic windowing	
							signalNew = this.calcParabolicWindowing(surrSignal1D); 
							//apply bridge dentrending
							signalNew = this.calcBridgeDetrending(surrSignal1D);
								
							resultPSD = betaPSD.computeRegression(signalNew, poweSpecLength/8, poweSpecLength/2);	
						}
...
//				//Compute specific values************************************************
				...
				//"PSD", "DISP", "SWV"
				if (this.choiceRadioButt_HurstType.equals("PSD_Beta")) {
					poweSpecLength = subSignal1D.length/2;	
					
					if (psdType.equals("PSD")){			
						resultPSD = betaPSD.computeRegression(subSignal1D, 1 , poweSpecLength);		
					}
					else if (psdType.equals("lowPSDwe")){
					
						//apply parabolic windowing	
						signalNew = this.calcParabolicWindowing(subSignal1D); 
						//apply bridge dentrending
						signalNew = this.calcBridgeDetrending(subSignal1D);
...
//				if (optShowPlot) {	
//					String preName = "PSD_Beta " +signalColumn.getHeader() + "-Box#" + (i+1);
...
//				}	
	 ...
	 */
	private double[] calcBridgeDetrending(double[] signal) {
		int length = signal.length;
		double[]signalBD = new double[length];
		double first = signal[0];
		double last = signal[length - 1];
		double step = (last - first)/(length - 1);
		for  (int i = 0; i < length; i++){
			double lineValue = first + (i * step);
			signalBD[i] = signal[i]- lineValue;
		}
		return signalBD;
	}
	 ...
	 */
	private double[] calcParabolicWindowing(double[] signal) {
		int length = signal.length;
		double[] signalW = new double[length];
		for  (int j = 0; j < length; j++){
			double b = (2.0d*(j+1))/(length+1) - 1;
			double parabWin = 1.0d - b*b;
			signalW[j] = signal[j]*parabWin;
		}
		return signalW;
	}
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(SignalHurst.class, true);
	}
}
