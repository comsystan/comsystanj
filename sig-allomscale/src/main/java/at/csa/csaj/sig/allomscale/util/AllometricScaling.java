package at.csa.csaj.sig.allomscale.util;
...
public class AllometricScaling {
	...
	public double[] calcLogRegression(double[][] asData, int regStart, int regEnd){
		...
		if (regEnd > asData[0].length){
			...
			logService.info(this.getClass().getName() + " AllometricScaling: Regression max > 1/4 signal size is not allowed");
			JOptionPane.showMessageDialog(null, "AllometricScaling: Regression max > 1/4 signal size is not allowed!", "Info", JOptionPane.INFORMATION_MESSAGE);
