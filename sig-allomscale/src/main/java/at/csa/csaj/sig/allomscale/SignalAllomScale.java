...
//	private final String labelBackgroundOptions = BACKGROUNDOPTIONS_LABEL;
	...
	protected void callbackProcessSingleColumn() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing Allometric scaling, please wait...<br>Open console window for further info.</html>");
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_AnalysisType.equals("Entire signal")){
			...
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				...
				tableResult.add(new DoubleColumn("Slope_Surr"));  //Mean surrogate value	
				...
				for (int s = 0; s < numSurrogates; s++) tableResult.add(new DoubleColumn("Slope_Surr-#"+(s+1))); 
...
//			}
	...
	private void writeToTable(int rowNumber, int signalNumber, double[] resultValues) {
		...
		tableResult.set(7, row, spinnerInteger_RegMin); //RegMin
		tableResult.set(8, row, spinnerInteger_RegMax); //RegMax	
		tableColLast = 8;
