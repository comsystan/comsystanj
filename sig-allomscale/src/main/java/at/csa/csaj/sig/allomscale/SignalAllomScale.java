 ...
 * #L%
 */
package at.csa.csaj.sig.allomscale;
...
import at.csa.csaj.sig.allomscale.util.AllometricScaling;
...
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Signal"),
	@Menu(label = "Allometric scaling ", weight = 340)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SignalAllomScale<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class SignalAllomScale<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	private Button buttonProcessAllColumns;
	...
	*/
	protected void startWorkflowForSingleColumn() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("Computing Allometric scaling, please wait... Open console window for further info.",
	...
	*/
	private double[] process(DefaultGenericTable dgt, int col) { //  c column number
		...
		if (signalRange.equals("Entire signal")){	
			...
			//if (signal1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
				...
				regressionValues = allomScale.calcLogRegression(meansAndVars, regMin, regMax);
				// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
				
				if (optShowPlot) {
					String preName = signalColumn.getHeader();
					showPlot(allomScale.getLnDataX(), allomScale.getLnDataY(), preName, col, regMin, regMax);	
				}	
				...
				if (!surrType.equals("No surrogates")) { //Add surrogate analysis
					...
					for (int s = 0; s < numSurrogates; s++) {
						...
						allomScale = new AllometricScaling(logService);
						meansAndVars = allomScale.calcMeansAndVariances(surrSignal1D);
						regressionValues = allomScale.calcLogRegression(meansAndVars, regMin, regMax);
						// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
						//According to West: FD = 2 - slope/2
						//But this did not match theoretical values.
		...
		//********************************************************************************************************	
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				//if (subSignal1D.length > (numWinSizeMax * 2)) { // only data series which are large enough
					allomScale = new AllometricScaling(logService);
					meansAndVars = allomScale.calcMeansAndVariances(subSignal1D);
					regressionValues = allomScale.calcLogRegression(meansAndVars, regMin, regMax);
					// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
	...
	private void showPlot(double[] lnDataX, double[] lnDataY, String preName, int col, int regMin, int regMax) {
		...
		RegressionPlotFrame doubleLogPlot = DisplayRegressionPlotXY(lnDataX, lnDataY, isLineVisible,
				"Double Log Plot - Allometric scaling", preName + "-" + tableInName, "ln(Mean)", "ln(Variance)", "", regMin, regMax);
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(SignalAllomScale.class, true);
	}
}
