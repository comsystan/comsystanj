 ...
 * #L%
 */
package at.csa.csaj.plugin1d.frac.hurst.hk;
...
import at.csa.csaj.plugin1d.frac.hurst.hk.util.HKprocess;
...
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Hurst coefficient (HK)", weight = 161)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SequenceHurstHK<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj1DHurstHK<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel

	private static final String PLUGIN_LABEL            = "<html><b>Hurst coefficient (HK)</b></html>";
	private static final String SPACE_LABEL             = "";
	private static final String HURSTOPTIONS_LABEL      = "<html><b>Hurst (HK) options</b></html>";
	...
	private static final String tableOutName = "Table - Hurst (HK)";
	...
	@Parameter(label = "n",
			   ...
			   initializer = "initialN", callback = "callbackN")
	private int spinnerInteger_N = 500;
...
//	private Button buttonProcessAllColumns;
	...
	protected void initialN() {
		spinnerInteger_N = 500;	
	}
	...
	/** Executed whenever the {@link #spinnerInteger_N} parameter changes. */
	protected void callbackN() {
		if (spinnerInteger_N < 100) {
		...
		logService.info(this.getClass().getName() + " n set to " + spinnerInteger_N);
	}
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SequenceRange.equals("Entire sequence")){
			...
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				...
				tableOut.add(new DoubleColumn("H_Surr")); //Mean surrogate value	
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("H_Surr-#"+(s+1))); 
			}	
		} 
		else if (choiceRadioButt_SequenceRange.equals("Subsequent boxes")){
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableOut.add(new DoubleColumn("H-#" + n));	
			}
		}
...
//			}
	...
	*/
	private double[] process(DefaultGenericTable dgt, int col) { //  c column number
		...
		numberN               = spinnerInteger_N;
		int boxLength         = spinnerInteger_BoxLength;
		int numDataPoints     = dgt.getRowCount();
		boolean skipZeroes    = booleanSkipZeroes;	
		boolean optShowPlot   = booleanShowDoubleLogPlot;
		
		double[] resultValues = new double[1]; // H
...
//		}
		...
		if (sequenceRange.equals("Entire sequence")){	
			if (surrType.equals("No surrogates")) {
				resultValues = new double[1]; // H
			} else {
				resultValues = new double[1+1+1*numSurrogates]; //+ Mean_Surr, Mean R2_Surr,  Dim_Surr1, Dim_Surr2,  Dim_Surr3, ......R2_Surr1,.... 2, 3......
			}
			for (int r = 0; r < resultValues.length; r++) resultValues[r] = Double.NaN;
			...
			hkProcess = new HKprocess();
			hkProcess.computeH(sequence1D, numberN);
			...
			if (!surrType.equals("No surrogates")) { //Add surrogate analysis
				...
				for (int s = 0; s < numSurrogates; s++) {
					...
					//Compute H with HK method
					hkProcess = new HKprocess();
					hkProcess.computeH(surrSequence1D, numberN);
					H = hkProcess.getH();
					
					resultValues[lastMainResultsIndex   + 1 + s]= H;
					//resultValues[lastMainResultsIndex + 1 + numSurrogates + s]    = ?;
					//resultValues[lastMainResultsIndex + 1 + (2*numSurrogates) +s] = ?;
					sumHs += H;
		...
		//********************************************************************************************************	
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				hkProcess.computeH(subSequence1D, numberN);
				H = hkProcess.getH();
		...
		//********************************************************************************************************			
			...
			for (int i = 0; i < actualNumGlidingBoxes; i++) {
				...
				resultValues[i]                            = H;  //H;
				//resultValues[(int)(i + numGlidingBoxes)] = ??; //??		
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Csaj1DHurstHK.class, true);
	}
}
