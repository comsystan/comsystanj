 ...
 * of a signal.
 */
@Plugin(type = InteractiveCommand.class, headless = true, menuPath = "Plugins>ComsystanJ>Signal>Permutation entropy")
...
//	private final String labelBackgroundOptions = BACKGROUNDOPTIONS_LABEL;
	...
	protected void callbackProcessSingleColumn() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing Permutation entropy, please wait...<br>Open console window for further info.</html>");
	...
	private void generateTableHeader() {
		...
		tableResult.add(new IntColumn("Order n"));
		...
		if (choiceRadioButt_AnalysisType.equals("Entire signal")){
			...
			tableResult.add(new DoubleColumn(entropySortingHeader));	
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				//do nothing	
			} else { //Surrogates
				tableResult.add(new DoubleColumn(entropyHeader+"_Surr"));  //Mean surrogate value	
	...
	private void writeToTable(int rowNumber, int signalNumber, double[] resultValues) {
		...
		tableResult.set(7, row, spinnerInteger_ParamN); // ParamN
		tableResult.set(8, row, spinnerInteger_ParamD); //ParamD		
		tableColLast = 8;
