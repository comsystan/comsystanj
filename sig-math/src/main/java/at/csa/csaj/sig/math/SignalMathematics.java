...
//		sliceLabels = new String[(int) numColumns];
	...
	private void deleteExistingDisplays() {
		...
		if (optDeleteExistingImgs) {
			...
			for (int i = listFrames.length -1 ; i >= 0; i--) { //Reverse order, otherwise focus is not given free from the last image
				...
				if (frame.getTitle().equals("Result signal(s)")) {
	...
	private void processSingleInputColumn (int s) throws InterruptedException {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			...
			SignalPlotFrame pdf = new SignalPlotFrame(tableResult, cols, isLineVisible, "Result signal(s)", signalTitle, xLabel, yLabel, seriesLabels);
