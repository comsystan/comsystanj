 ...
 * File: Csaj1DFractalDimensionFragmentation.java
 * 
 * $Id$
 * $HeadURL$
 * 
 ...
 * #L%
 */
package at.csa.csaj.plugin1d.frac.dim.frag;
...
import at.csa.csaj.plugin1d.frac.dim.frag.util.FragmentationDimension;
...
/**
 * A {@link Command} plugin computing <the Fragmentation index</a>
...
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Fragmentation dimension ", weight = 215)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class Csaj1DFractalDimensionFragmentation<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj1DFractalDimensionFragmentation<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
	...
	private static int numRegMax = 1000;
	
	private static ArrayList<RegressionPlotFrame> doubleLogPlotList = new ArrayList<RegressionPlotFrame>();
	
	private static final String tableOutName = "Table - Fragmentation dimension";
...
//	private Button buttonProcessAllColumns;
	...
	protected void initialRegMax() {
		//numRegMax = (int) Math.floor(tableIn.getRowCount() / 2.0);
		//numRegMax is already set in method checkItemIO;
		spinnerInteger_RegMax = numRegMax;
	...
	protected void callbackRegMax() {
		...
		if (spinnerInteger_RegMax > numRegMax) spinnerInteger_RegMax = numRegMax; 
	...
	*/
	protected void startWorkflowForSingleColumn() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("Computing Fragmentation dimensions, please wait... Open console window for further info.",
...
//			}
	...
	private void processSingleInputColumn (int c) {
		...
		// 0 D, 1 R2, 2 StdErr
		logService.info(this.getClass().getName() + " Fragmentation dimension: " + resultValues[0]);
	...
	private void writeToTable(int rowNumber, int sequenceNumber, double[] resultValues) {
		...
		tableOut.set(7, row, spinnerInteger_RegMin); //RegMin
	...
	*/
	private double[] process(DefaultGenericTable dgt, int col) { //  c column number
		...
		if (sequenceRange.equals("Entire sequence")){	
			...
			if (sequence1D.length > 2) { // only data series which are large enough
				...
				regressionValues = fragD.calcDimension(sequence1D, regMin, regMax);
				// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
				
				if (optShowPlot) {
					String preName = sequenceColumn.getHeader();
					showPlot(fragD.getLnDataX(), fragD.getLnDataY(), preName, col, regMin, regMax);
				}	
				...
				if (!surrType.equals("No surrogates")) { //Add surrogate analysis
					...
					for (int s = 0; s < numSurrogates; s++) {
						...
						fragD = new FragmentationDimension();
						regressionValues = fragD.calcDimension(surrSequence1D, regMin, regMax);
						// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
		...
		//********************************************************************************************************	
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				if (subSequence1D.length > 2) { // only data series which are large enough
					fragD = new FragmentationDimension();
					regressionValues = fragD.calcDimension(subSequence1D, regMin, regMax);
					// 0 Intercept, 1 Slope, 2 InterceptStdErr, 3 SlopeStdErr, 4 RSquared
	...
	private void showPlot(double[] lnDataX, double[] lnDataY, String preName, int col, int regMin, int regMax) {
		...
		RegressionPlotFrame doubleLogPlot = DisplayRegressionPlotXY(lnDataX, lnDataY, isLineVisible,
				"Double Log Plot - Fragmentation Dimension", preName + "-" + tableInName, "ln(x)", "ln(N)", "", regMin, regMax);
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Csaj1DFractalDimensionFragmentation.class, true);
	}
}
