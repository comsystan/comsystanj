/*-
 * #%L
 * Project: ImageJ2 sequence plugin for Filtering
...
package at.csa.csaj.plugin1d.filter;
...
@Plugin(type = ContextCommand.class,
	...
	@Menu(label = "Filter ", weight = 50)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SequenceFilter<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Plugin1DFilter<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
	...
	private void processSingleInputColumn (int s) {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			if (resultValues != null) { 
				...
				SequencePlotFrame pdf = new SequencePlotFrame(tableOut, cols, isLineVisible, "Filtered sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
...
//		}
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Plugin1DFilter.class, true);
	}
}
