 ...
 * File: Higuchi2D_RGB_MultDiff.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class Higuchi2D_RGB_MultDiff implements Higuchi2DMethods{
	 ...
	 */
	public Higuchi2D_RGB_MultDiff(RandomAccessibleInterval<?> rai, int numK, boolean skipZeroes)  {
		this.rai   = rai;
		this.numK  = numK;
		this.skipZeroes = skipZeroes;
	}

	public Higuchi2D_RGB_MultDiff() {
	}
...
//					        //H_blue=array2D_2[m1+(i+1)*k-1][m2+(j-1)*k-1];
								...
								if (!skipZeroes) { //no skipping
		                            ...
		                            L_vec[mm] += (sumDiff_Blue/2.0 + sumDiff_Red/2.0 + sumDiff_Green/2.0);// * norm;
