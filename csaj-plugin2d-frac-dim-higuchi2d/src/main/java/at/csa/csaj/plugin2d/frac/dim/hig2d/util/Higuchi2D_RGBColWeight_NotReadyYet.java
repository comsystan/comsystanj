 ...
 * File: Higuchi2D_RGBColWeight_NotReadyYet.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class Higuchi2D_RGBColWeight_NotReadyYet implements Higuchi2DMethods{
	 ...
	 */
	public Higuchi2D_RGBColWeight_NotReadyYet(RandomAccessibleInterval<?> rai, int numK, boolean skipZeroes)  {
		this.rai   = rai;
		this.numK     = numK;
		this.skipZeroes = skipZeroes;
	}

	public Higuchi2D_RGBColWeight_NotReadyYet() {
	}

	/**
	 * This method calculates the color weighted Higuchi dimension for 2D RGB images
	 * @return totals
	 */
	@Override
	public double[] calcTotals() {
		...
		//System.out.println("Higuchi2D_ColorWeighted: Preparing integer stack...");
...
//		}
		...
		long average_red_sum   = 0;
		long average_green_sum = 0;
		long average_blue_sum  = 0;
			
		//calculate mean of each band
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
					average_red_sum   += (array2D_0[x][y]);
					average_green_sum += (array2D_1[x][y]);
					average_blue_sum  += (array2D_2[x][y]);
			}
		}	
		average_red   = (double)average_red_sum   / (width*height);
		average_green = (double)average_green_sum / (width*height);
		average_blue  = (double)average_blue_sum  / (width*height);
		...
		for (int k = 1; k <=numK; k++) {
			...
			totals[k-1] = Math.pow(L_Sum/((double)(k*k)), psi);		
		} //k
	...
	/**
	 * This method calculates the number of k's
	 * @return eps
	 */
	@Override
