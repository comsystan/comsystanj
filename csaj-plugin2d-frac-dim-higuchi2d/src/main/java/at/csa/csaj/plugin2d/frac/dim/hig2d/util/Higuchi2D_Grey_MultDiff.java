 ...
 * File: Higuchi2D_Grey_MultDiff.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class Higuchi2D_Grey_MultDiff implements Higuchi2DMethods{
	 ...
	 */
	public Higuchi2D_Grey_MultDiff(RandomAccessibleInterval<?> rai, int numK, boolean skipZeroes) {
		this.rai = rai;
		this.numK = numK;
		this.skipZeroes = skipZeroes;
	}

	public Higuchi2D_Grey_MultDiff() {
	}
	...
	public double[] calcTotals() {
		...
		//System.out.println("Higuchi2D_MultDiff_HA: Preparing integer array...");
...
//								L_vec[mm] += (Math.abs(A-C)*Math.abs(A-E)+
//										      Math.abs(Z-C)*Math.abs(Z-E))/2.0; // * norm;	
								L_vec[mm] += (Math.abs(A-C)*Math.abs(A-E));
