 ...
 * File: Higuchi2D_RGB_KfoldDiff.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class Higuchi2D_RGB_KfoldDiff implements Higuchi2DMethods{
	 ...
	 */
	public Higuchi2D_RGB_KfoldDiff(RandomAccessibleInterval<?> rai, int numK, boolean skipZeroes)  {
		this.rai   = rai;
		this.numK  = numK;
		this.skipZeroes = skipZeroes;
	}

	public Higuchi2D_RGB_KfoldDiff() {
	}
...
//					        //H_blue=array2D_2[m1+(i+1)*k-1][m2+(j-1)*k-1];
								...
								if (!skipZeroes) { //no skipping
							        sumDiff_Red = Math.abs(Z_red-C_red)*k +
						        			      Math.abs(A_red-C_red)*k +
				        		                  Math.abs(Z_red-E_red)*k +
				        		                  Math.abs(A_red-E_red)*k;
							        
							        sumDiff_Green = Math.abs(Z_green-C_green)*k +
							        		        Math.abs(A_green-C_green)*k +
				        		                    Math.abs(Z_green-E_green)*k +
				        		                    Math.abs(A_green-E_green);
							        
							        sumDiff_Blue = Math.abs(Z_blue-C_blue)*k +
							        		       Math.abs(A_blue-C_blue)*k +
							        		       Math.abs(Z_blue-E_blue)*k +
							        		       Math.abs(A_blue-E_blue);
