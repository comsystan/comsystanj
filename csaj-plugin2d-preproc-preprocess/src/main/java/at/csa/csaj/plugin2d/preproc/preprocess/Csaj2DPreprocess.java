/*-
 * #%L
 * Project: ImageJ2 plugin for preprocessing.
 * File: Csaj2DPreprocess.java
 * 
 * $Id$
 * $HeadURL$
 * 
 ...
 * #L%
 */
package at.csa.csaj.plugin2d.preproc.preprocess;
...
/**
 * A {@link Command} plugin for <preprocessing</a>
 * of an image.
 */
@Plugin(type = ContextCommand.class,
		...
		@Menu(label = "Preprocess", weight = 22)})
//public class Img2DPreprocess<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj2DPreprocess<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel

	private static final String PLUGIN_LABEL            = "<html><b>Preprocessing</b></html>";
	private static final String SPACE_LABEL             = "";
	private static final String PREPROCESSOPTIONS_LABEL = "<html><b>Preprocessing options</b></html>";
...
//	private final String labelSpace = SPACE_LABEL;
	...
	//-----------------------------------------------------------------------------------------------------
	@Parameter(label = " ", visibility = ItemVisibility.MESSAGE, persist = false)
	private final String labelPreprocessOptions = PREPROCESSOPTIONS_LABEL;
	
	@Parameter(label = "Type of preprocessing",
			   description = "Choose one of the options",
			   style = ChoiceWidget.RADIO_BUTTON_VERTICAL_STYLE,
			   choices = {"Auto crop borders", "Particles to stack"}, //
			   persist = true,  //restore previous value default = true
			   initializer = "initialPreprocessType",
	...
	@Parameter(label = "(PTS) Connectivity",
...
//	private Button buttonProcessAllImages;
	...
	protected void initialPreprocessType() {
		choiceRadioButt_PreprocessType = "Auto crop borders";
	} 
	...
	// ------------------------------------------------------------------------------
	/** Executed whenever the {@link #choiceRadioButt_PreprocessType} parameter changes. */
	protected void callbackPreprocessType() {	
		if (numSlices > 1) { //Particles to stack only for single input image
			JOptionPane.showMessageDialog(null, "Single input image expected\nParticles to stack not possible for input image stack", "Warning", JOptionPane.WARNING_MESSAGE);
			choiceRadioButt_PreprocessType = "Auto crop borders";
		}
		logService.info(this.getClass().getName() + " Preprocess type set to " + choiceRadioButt_PreprocessType);
	}
...
//			datasetPreview.setColorTable(ColorTables.BLUE,  2);
	...
	*/
	protected void startWorkflowForSingleImage() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("Preprocessing, please wait... Open console window for further info.",
...
//		}
		...
		if (preprocessType.equals("Auto crop borders")) { //Result image will be smaller
		...
		else if (preprocessType.equals("Particles to stack")) { //Result is an image stack 
			//do nothing
	...
	//			RandomAccessibleInterval< FloatType > rai = Views.interval(imgFloat, new long[] { 0, 0 }, new long[] { imgFloat.max(0), imgFloat.max(1) });
				...
				if (choiceRadioButt_PreprocessType.equals("Auto crop borders")) {
...
//				labeledParticles = ArrayImgs.unsignedBytes(dimensions);
	...
	public static void main(final String... args) throws Exception {
		...
		ij.command().run(Csaj2DPreprocess.class, true);
	}
}
