/*-
 * #%L
 * Project: ImageJ2 sequence plugin to cut out (shorten) sequences
...
package at.csa.csaj.plugin1d.cutout;
...
/**
 * A {@link Command} plugin for cutting out a sub-sequence.
 */
@Plugin(type = ContextCommand.class,
	...
	@Menu(label = "Cut out ", weight = 20)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SequenceCutOut<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Plugin1DCutOut<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	private static long numGlidingBoxes = 0;
	...
	private static ArrayList<SequencePlotFrame> plotDisplayFrameList = new ArrayList<SequencePlotFrame>();
...
//		sliceLabels = new String[(int) numColumns];
	...
	*/
	protected void startWorkflowForSingleColumn() {
		dlgProgress = new WaitingDialogWithProgressBar("Cutting out a sub-sequence, please wait... Open console window for further info.",
	...
	private void processSingleInputColumn (int s) {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			if (resultValues != null) { 
				...
				SequencePlotFrame pdf = new SequencePlotFrame(tableOut, cols, isLineVisible, "Sub-sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
	...
	private void writeToTable(int sequenceNumber, double[] resultValues) {
		...
		if (resultValues == null) {
			...
			for (int r = 0; r < resultValues.length; r++ ) {
				tableOut.set(sequenceNumber, r, resultValues[r]);	
			}
	...
	*/
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Plugin1DCutOut.class, true);
	}
}
