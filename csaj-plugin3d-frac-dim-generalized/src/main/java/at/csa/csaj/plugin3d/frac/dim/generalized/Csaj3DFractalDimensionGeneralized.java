 ...
 * File: Csaj3DFractalDimensionGeneralized.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
package at.csa.csaj.plugin3d.frac.dim.generalized;
...
/**
 * A {@link ContextCommand} plugin computing <the 3D Generalized dimensions</a>
 * of an image volume.
 */
@Plugin(type = ContextCommand.class,
headless = true,
label = "3D Generalized dimensions",
...
@Menu(label = "3D Generalized dimensions", weight = 40)})
//public class Csaj3DFractalDimensionGeneralized<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj3DFractalDimensionGeneralized<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel

	private static final String PLUGIN_LABEL            = "Computes 3D Generalized dimensions";
	...
	private static ArrayList<SequencePlotFrame> genDimPlotList      = new ArrayList<SequencePlotFrame>();
	private static ArrayList<SequencePlotFrame> fSpecPlotList       = new ArrayList<SequencePlotFrame>();
	private static double[][] resultValuesTable; //First column is q, the second column are the corresponding regression values
	private static final String tableOutName = "Table - 3D Generalized dimensions";
...
//	}
	...
	*/
	protected void startWorkflowForSingleVolume() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("Computing 3D Generalized dimensions, please wait... Open console window for further info.",
...
//			}
	...
	private void processSingleInputVolume() {
		...
		resultValuesTable = new double[numQ][10];
		...
		for (int q = 0; q < numQ; q++) {
			for (int i = 0; i < regressionValues[0].length; i++ ) {
					resultValuesTable[q][i] = regressionValues[q][i]; 
			}
			//Compute dimension
	...
	*/
	private double[][] process(RandomAccessibleInterval<?> rai) { //3Dvolume
		...
		String plot_method = "3D Generalized dimensions";
		String xAxis = "ln(eps)";
		String yAxis = "ln(Count)";
		
		GeneralizedDim3DMethods gd3D = null;
		
		if (imageType.equals("Grey")) {// grey image   //additional check, is already checked during validation of active dataset
			//*****************************************************************************************************************************************
			gd3D = new GeneralizedDim3D_Grey(rai, numBoxes, minQ, maxQ, scanningType, colorModelType, pixelPercentage, dlgProgress, statusService);	
			plot_method="3D Generalized dimensions";
			xAxis = "ln(Box size)";
			yAxis = "ln(Count)";
		...
		if (optShowDoubleLogPlot) {
			...
			if ((imageType.equals("Grey")) || (imageType.equals("RGB"))) { //both are OK
				...
				RegressionPlotFrame doubleLogPlot = DisplayMultipleRegressionPlotXY(lnEps, lnTotals, isLineVisible, "Double Log Plot - Generalized dimensions", 
		...
		for (int q = 0; q < numQ; q++) {
			...
			regressionParams[q] = lr.calculateParameters(lnEps, lnTotals[q], regMin, regMax);
		...
		if (optShowDqPlot) {	
			...
			SequencePlotFrame dimGenPlot = DisplaySinglePlotXY(qList, genDimList, isLineVisible, "Generalized dimensions", 
		...
		// Output
		// uiService.show("Table - 3D Generalized dimensions", table);
	}
	...
	public static void main(final String... args) throws Exception {
		...
		// ij.command().run(Csaj3DFractalDimensionGeneralized.class,
		// true).get().getOutput("image");
		ij.command().run(Csaj3DFractalDimensionGeneralized.class, true);
	}
}
