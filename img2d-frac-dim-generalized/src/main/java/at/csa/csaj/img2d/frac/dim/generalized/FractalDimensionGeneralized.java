...
@Plugin(type = InteractiveCommand.class, 
        headless = true,
	    menuPath = "Plugins>ComsystanJ>Image(2D)>Fractal dimension - Generalized")
...
//	}
	...
	protected void callbackProcessActiveImage() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing Generalized dimensions, please wait...<br>Open console window for further info.</html>");
...
//			//This dose not work with DisplayService because the JFrame is not "registered" as an ImageJ display	
	...
	private void generateTableHeader(){
		...
		int numQ = numMaxQ - numMinQ + 1;
		for (int q = 0; q < numQ; q++) {
			table.add(new GenericColumn("D" + (numMinQ + q)));
		}
	}
	...
	private void collectActiveResultAndShowTable(int sliceNumber) {
	    ...
	    int s = sliceNumber;	
			...
			int numQ = numMaxQ - numMinQ + 1;
			for (int q = 0; q < numQ; q++) {
				table.set("D" + (numMinQ + q), table.getRowCount()-1, resultValuesTable[q][s][1]);
			}	
