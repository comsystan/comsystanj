/*-
 * #%L
 * Project: ImageJ2 sequence plugin for computing fractal dimension with the Tug of war algorithm
 * File: Plugin1DFractalDimensionTugOfWar.java
 * 
 * $Id$
 * $HeadURL$
 * 
 ...
 * #L%
 */
package at.csa.csaj.plugin1d.frac.dim.tow;
...
import at.csa.csaj.plugin1d.frac.dim.tow.util.TugOfWar;
import at.csa.csaj.plugin1d.open.Plugin1DOpener;

/**
 * A {@link Command} plugin computing <the Tug of war dimension</a>
 * of a  sequence.
 */
...
//public class SequenceFractalDimensionTugOfWar<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Plugin1DFractalDimensionTugOfWar<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	private Button buttonProcessAllColumns;
	...
	protected void initialNumBoxes() {
		//numbNumBoxes = (int) Math.floor(tableIn.getRowCount() / 3.0);
	...
	*/
		...
		if (sequenceRange.equals("Entire sequence")){	
			...
			if (sequence1D.length > (numBoxes * 2)) { // only data series which are large enough
				...
				if (optShowPlot) {
					...
					showPlot(tow.getLnDataX(), tow.getLnDataY(), preName, col, regMin, regMax);
				}	
	...
	private void showPlot(double[] lnDataX, double[] lnDataY, String preName, int col, int regMin, int regMax) {
		...
		RegressionPlotFrame doubleLogPlot = DisplayRegressionPlotXY(lnDataX, lnDataY, isLineVisible,
				"Double Log Plot - Tug of war dimension", preName + "-" + tableInName, "ln(box width)", "ln(Totals)", "", regMin, regMax);
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Plugin1DFractalDimensionTugOfWar.class, true);
	}
}
