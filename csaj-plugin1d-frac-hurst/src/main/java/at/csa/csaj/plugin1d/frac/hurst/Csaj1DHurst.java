...
import at.csa.csaj.commons.utils.GenerateInterval;
...
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Hurst coefficient (HK,RS,SP)", weight = 160)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
...
public class Csaj1DHurst<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
	...
	private static int   epsMin;
	private static int   epsMax;
	...
	@Parameter(label = "(RS) Win Min",
			   ...
			   stepSize = "1",
			   persist = false, //restore previous value default = true
			   initializer = "initialEpsMin", callback = "callbackEpsMin")
	private int spinnerInteger_EpsMin = 1;
	...
	@Parameter(label = "(RS) Win Max",
			   ...
			   stepSize = "1",
			   persist = false, //restore previous value default = true
			   initializer = "initialEpsMax", callback = "callbackEpsMax")
	private int spinnerInteger_EpsMax = 3;
...
//	private Button buttonProcessAllColumns;
	...
	protected void initialEpsMin() {
		epsMin = 2;
		spinnerInteger_EpsMin = epsMin;
	}
	protected void initialEpsMax() {
		epsMax = (int) Math.floor(tableIn.getRowCount() / 1.0);
		spinnerInteger_EpsMax = epsMax;
	}
	protected void initialEpsN() {
		if (epsMax - epsMin + 1  < 10) {
			epsN = epsMax - epsMin + 1;
		} else {
			epsN = 10;
		}
	...
	/** Executed whenever the {@link #spinnerInteger_EpsMin} parameter changes. */
	protected void callbackEpsMin() {
		
		if (spinnerInteger_EpsMin >= spinnerInteger_EpsMax - 2) {
			spinnerInteger_EpsMin = spinnerInteger_EpsMax - 2;
		}
		if (spinnerInteger_EpsMin < 1) {
			spinnerInteger_EpsMin = 1;
		}
		epsMin = spinnerInteger_EpsMin;
		logService.info(this.getClass().getName() + " Regression Min set to " + spinnerInteger_EpsMin);
		callbackEpsN();
	}
	/** Executed whenever the {@link #spinnerInteger_EpsMax} parameter changes. */
	protected void callbackEpsMax() {
		if (spinnerInteger_EpsMax <= spinnerInteger_EpsMin + 2) {
			spinnerInteger_EpsMax = spinnerInteger_EpsMin + 2;
		}
		if (spinnerInteger_EpsMax > numRows) {
			spinnerInteger_EpsMax = (int)numRows;
		}
		epsMax = spinnerInteger_EpsMax;
		logService.info(this.getClass().getName() + " Regression Max set  to " + spinnerInteger_EpsMax);
		callbackEpsN();
	}
	/** Executed whenever the {@link #spinnerInteger_EpsN} parameter changes. */
	protected void callbackEpsN() {
		if (spinnerInteger_EpsMax - spinnerInteger_EpsMin + 1 < spinnerInteger_EpsN) {
			spinnerInteger_EpsN = spinnerInteger_EpsMax - spinnerInteger_EpsMin + 1;
		}
		epsN = spinnerInteger_EpsN;
		epsInterval = GenerateInterval.getIntLogDistributedInterval(epsMin, epsMax, epsN);
	...
	*/
	protected void startWorkflowForSingleColumn() {
		...
		dlgProgress = new WaitingDialogWithProgressBar("Computing Hurst value(s), please wait... Open console window for further info.",
...
//			}
	...
	private void writeToTable(int rowNumber, int sequenceNumber, double[] resultValues) {
		...
		if (booleanHurstRS) {
			tableOut.set(8,  row, epsMin); //may be changed by the algorithm
			tableOut.set(9,  row, epsMax); //may be changed by the algorithm
	...
	*/
	private double[] process(DefaultGenericTable dgt, int col) { //  c column number
		...
		epsMin                = spinnerInteger_EpsMin; //may be changed later
		epsMax                = spinnerInteger_EpsMax; //may be changed later
...
//					}
	...
	private void showPlot(double[] lnDataX, double[] lnDataY, String preName, int col, String labelX, String labelY, int epsMin, int epsMax) {
		if (lnDataX == null) {
		...
		if (epsMin >= epsMax) {
			logService.info(this.getClass().getName() + " epsMin >= epsMax, cannot display the plot!");
			return;
		}
		if (epsMax <= epsMin) {
			logService.info(this.getClass().getName() + " epsMax <= epsMin, cannot display the plot!");
			return;
		}
		// String preName = "";
		if (preName == null) {
		...
		RegressionPlotFrame doubleLogPlot = DisplayRegressionPlotXY(lnDataX, lnDataY, isLineVisible,
				"Double Log Plot - ", preName + "-" + tableInName, labelX, labelY, "", epsMin, epsMax);
	 ...
	 * @param epsMin                minimum value for regression range
	 * @param epsMax                maximal value for regression range
