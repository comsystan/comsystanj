 ...
 * #L%
 */
package at.csa.csaj.plugin1d.open;
...
@Plugin(type = ContextCommand.class,
	...
	@Menu(label = "Sequence opener ", weight = 1)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
public class Plugin1DOpener<T extends RealType<T>> extends ContextCommand { //modal GUI with cancel
    ...
    public void run() {
		...
		if ((files.length == 1) && (files[0] != null)){
			...
			try {
				...
				logService.info(this.getClass().getName() + " WARNING #SO31: It was not possible to load the sequence");
				e.printStackTrace();
				dlgProgress.setVisible(false);
				dlgProgress.dispose();
				return;
			} catch (NumberFormatException nfe) {
				logService.info(this.getClass().getName() + " WARNING #SO32: It was not possible to load the sequence");
				nfe.printStackTrace();
				dlgProgress.setVisible(false);
				dlgProgress.dispose();
				return;
			...
			if (numColumns == 1) {
				...
				if (selectedOption == JOptionPane.YES_OPTION) {
					pdf = new SequencePlotFrame(defaultGenericTable, 0, isLineVisible, "Sequence(s)", sequenceTitle, xLabel, yLabel);
					pdf.setVisible(true);
				}
			...
			if (numColumns > 1) {
				...
				int selectedOption = JOptionPane.showConfirmDialog(null, "Do you want to display the sequences?\nNot recommended for a large number of sequences", "Display option", JOptionPane.YES_NO_OPTION); 
				if (selectedOption == JOptionPane.YES_OPTION) {
					pdf = new SequencePlotFrame(defaultGenericTable, cols, isLineVisible, "Sequence(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
					pdf.setVisible(true);
				}		
...
//       
         //invoke the plugin
         ij.command().run(Plugin1DOpener.class, true);
