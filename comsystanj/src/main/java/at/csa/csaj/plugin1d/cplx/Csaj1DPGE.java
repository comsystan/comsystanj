...
/**
 * A {@link ContextCommand} plugin computing <Porta, Guzik, and Ehler indices</a>
* of a sequence.
 * Use distances from the diagonal line in the Poincare plot
...
@Plugin(type = ContextCommand.class, 
	headless = true,
	label = "Porta Guzik Ehler indices",
	...
	@Menu(label = "Porta Guzik Ehler indices ")}) //Space at the end of the label is necessary to avoid duplicate with 2D plugin 
//public class Csaj1DPGE<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Csaj1DPGE<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel


	private static final String PLUGIN_LABEL            = "<html><b>Porta Guzik Ehler indices</b></html>";
	private static final String SPACE_LABEL             = "";
	//private static final String REGRESSION_LABEL      = "<html><b>Regression parameters</b></html>";
	private static final String POINCARE_LABEL          = "<html><b>Poincare plot parameters</b></html>";
...
//    private String choiceRadioButt_Algorithm;
	...
	//-----------------------------------------------------------------------------------------------------
	@Parameter(label = " ", visibility = ItemVisibility.MESSAGE, persist = false)
	private final String labelPhaseSpaceOptions = POINCARE_LABEL;
...
//	}
	...
	*/
	protected void startWorkflowForSingleColumn() {
		...
		dlgProgress = new Dialog_WaitingWithProgressBar("Computing Porta Guzik Ehler indices, please wait... Open console window for further info.",
	...
	*/
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SequenceRange.equals("Entire sequence")){
			tableOut.add(new DoubleColumn("Porta"));	
			tableOut.add(new DoubleColumn("Guzik"));
			tableOut.add(new DoubleColumn("Ehler"));
			
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				//do nothing	
			} else { //Surrogates
				tableOut.add(new DoubleColumn("Porta_Surr"));   //Mean surrogate value	
				tableOut.add(new DoubleColumn("Guzik_Surr"));  //Mean surrogate value
				tableOut.add(new DoubleColumn("Ehler_Surr"));  //Mean surrogate value
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("Porta_Surr-#"+(s+1))); 
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("Guzik_Surr-#"+(s+1))); 
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("Ehler_Surr-#"+(s+1))); 
			}	
		} 
		else if (choiceRadioButt_SequenceRange.equals("Subsequent boxes")){
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableOut.add(new DoubleColumn("Porta-#" + n));	
			}
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableOut.add(new DoubleColumn("Guzik-#" + n));	
			}
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableOut.add(new DoubleColumn("Ehler-#" + n));	
			}
		}
...
//			}
	...
	private void processSingleInputColumn (int c) {
		...
		// 0 Porta, 1 Guzik, 2 Ehler
		if (containerPM != null) {
			logService.info(this.getClass().getName() + " Porta index: " + containerPM.item1_Values[0]);
			logService.info(this.getClass().getName() + " Guzik index: " + containerPM.item1_Values[1]);
			logService.info(this.getClass().getName() + " Ehler index: " + containerPM.item1_Values[2]);
		}
	...
	private void writeToTable(int numRow, int sequenceNumber, Container_ProcessMethod containerPM) {
		...
		tableOut.set(7, row, spinnerInteger_Tau); //Tau	
	...
	*/
	private Container_ProcessMethod process(DefaultGenericTable dgt, int col) { //  c column number
		...
		double[] resultValues = new double[3]; // // 0 Porta, 1 Guzik, 2 Ehler
		...
		PortaGuzikEhler pge;
		double[] pgeValues = null;

		//"Entire sequence", "Subsequent boxes", "Gliding box" 
		//********************************************************************************************************
		if (sequenceRange.equals("Entire sequence")){	
			if (surrType.equals("No surrogates")) {
				resultValues = new double[3]; // Porta Guzig Ehler
			} else {
				resultValues = new double[3+3+3*numSurrogates]; // P_Surr, G_Surr, E_Surr, P_Surr1, P_Surr2, P_Surr3,...Surr2...Surr3...
			}
			for (int r = 0; r < resultValues.length; r++) resultValues[r] = Double.NaN;
			...
			if (sequence1D.length > (tau * 2)) { // only data series which are large enough
				...
				pge = new PortaGuzikEhler();
				pgeValues = pge.calcIndices(sequence1D, tau); //Porta Guzik Ehler indices		
				resultValues[0] = pgeValues[0]; //Porta index
				resultValues[1] = pgeValues[1]; //Guzik index
				resultValues[2] = pgeValues[2]; //Ehler index
				...
				if (!surrType.equals("No surrogates")) { //Add surrogate analysis
					...
					for (int s = 0; s < numSurrogates; s++) {
						...
						pge = new PortaGuzikEhler();
						pgeValues = pge.calcIndices(surrSequence1D, tau); //Porta Guzik Ehler indices
				
						resultValues[lastMainResultsIndex + 4 +                 + s] = pgeValues[0];
						resultValues[lastMainResultsIndex + 4 +   numSurrogates + s] = pgeValues[1];
						resultValues[lastMainResultsIndex + 4 + 2*numSurrogates + s] = pgeValues[2];
	
						sumPorta += pgeValues[0];
						sumGuzik += pgeValues[1];
						sumEhler += pgeValues[2];
					}
					resultValues[lastMainResultsIndex + 1] = sumPorta/numSurrogates;
					resultValues[lastMainResultsIndex + 2] = sumGuzik/numSurrogates;
					resultValues[lastMainResultsIndex + 3] = sumEhler/numSurrogates;
				}	
			} 
		//********************************************************************************************************	
		} else if (sequenceRange.equals("Subsequent boxes")){
			resultValues = new double[(int) (3*numSubsequentBoxes)]; // Porta Guzig Ehler  == 3* number of boxes		
			for (int r = 0; r<resultValues.length; r++) resultValues[r] = Double.NaN;
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				pge = new PortaGuzikEhler();
				pgeValues = pge.calcIndices(subSequence1D, tau); //Porta Guzik Ehler indices		
				
				resultValues[i]                               = pgeValues[0]; //Porta
				resultValues[(int)(i +   numSubsequentBoxes)] = pgeValues[1]; //Guzik
				resultValues[(int)(i + 2*numSubsequentBoxes)] = pgeValues[2]; //Ehler			
				//***********************************************************************
			}	
		//********************************************************************************************************			
		} else if (sequenceRange.equals("Gliding box")){
			resultValues = new double[(int) (3*numGlidingBoxes)]; // Porta Guzig Ehler == 3 * number of boxes	
			for (int r = 0; r<resultValues.length; r++) resultValues[r] = Double.NaN;
			subSequence1D = new double[(int) numBoxLength];
			...
			for (int i = 0; i < actualNumGlidingBoxes; i++) {
				...
				resultValues[i]                            = pgeValues[0]; //Porta
				resultValues[(int)(i +   numGlidingBoxes)] = pgeValues[1]; //Guzik
				resultValues[(int)(i + 2*numGlidingBoxes)] = pgeValues[2]; //Ehler			
				//***********************************************************************
			}
		}
	...
	private void showPlot(double[] dataX, double[] dataY, String preName, int col, int numRegStart, int numRegEnd) {
		if (dataX == null) {
	...
	// This method shows the double log plot
	private void showPlots(double[] dataX, double[][] dataY, String preName, int col, int numRegStart, int numRegEnd) {
		if (dataX == null) {
			logService.info(this.getClass().getName() + " lnDataX == null, cannot display the plot!");
			return;
		}
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Csaj1DPGE.class, true);
	}
}
