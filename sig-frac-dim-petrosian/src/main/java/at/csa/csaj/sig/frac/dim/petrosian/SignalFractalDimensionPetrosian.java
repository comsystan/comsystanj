/*-
 * #%L
 * Project: ImageJ signal plugin for computing fractal dimension with Petrosian algorithm.
 * File: SignalFractalDimensionPetrosian.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
package at.csa.csaj.sig.frac.dim.petrosian;
...
import at.csa.csaj.sig.frac.dim.petrosian.util.Petrosian;
import at.csa.csaj.sig.open.SignalOpener;

/**
 * A {@link Command} plugin computing <the Petrosian dimension</a>
 * of a  signal.
 */
...
public class SignalFractalDimensionPetrosian<T extends RealType<T>> extends InteractiveCommand implements Command, Previewable { // non blocking  GUI
//public class SignalFractalDimensionPetrosian<T extends RealType<T>> implements Command {	//modal GUI
...
//	private final String labelBackgroundOptions = BACKGROUNDOPTIONS_LABEL;
	...
	protected void callbackProcessSingleColumn() {
		...
		//dlgProgress = new WaitingDialogWithProgressBar("<html>Computing Petrosian dimensions, please wait...<br>Open console window for further info.</html>");
		dlgProgress = new WaitingDialogWithProgressBar("Computing Petrosian dimensions, please wait... Open console window for further info.",
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SignalRange.equals("Entire signal")){
			tableResult.add(new DoubleColumn("Dpet"));	
			
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				//do nothing	
			} else { //Surrogates
				tableResult.add(new DoubleColumn("Dpet_Surr")); //Mean surrogate value	
			
				for (int s = 0; s < numSurrogates; s++) tableResult.add(new DoubleColumn("DpetSurr-#"+(s+1))); 
			}	
		} 
		else if (choiceRadioButt_SignalRange.equals("Subsequent boxes")){
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableResult.add(new DoubleColumn("Dpet-#" + n));	
			}
		}
...
//		}
		...
		if (signalRange.equals("Entire signal")){	
			...
			Dpet = petrosian.calcDimension(signal1D, criterion);
			...
			if (!surrType.equals("No surrogates")) { //Add surrogate analysis
				...
				for (int s = 0; s < numSurrogates; s++) {
					...
					petrosian = new Petrosian();
					Dpet = petrosian.calcDimension(surrSignal1D, criterion);
		...
		//********************************************************************************************************	
			...
			for (int i = 0; i < actualNumSubsequentBoxes; i++) {	
				...
				petrosian = new Petrosian();
				Dpet = petrosian.calcDimension(subSignal1D, criterion);
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(SignalFractalDimensionPetrosian.class, true);
	}
}
