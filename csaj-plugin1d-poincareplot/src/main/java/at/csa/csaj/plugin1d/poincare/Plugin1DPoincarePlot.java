/*-
 * #%L
 * Project: ImageJ2 sequence plugin for generating a Poincare plot
...
package at.csa.csaj.plugin1d.poincare;
...
//public class SequencePoincarePlot<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Plugin1DPoincarePlot<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//		}
	...
	private void processSingleInputColumn (int s) {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			...
			SequencePlotFrame pdf = new SequencePlotFrame(sequence1D, dataY, isLineVisible, "Poincare plot(s)", sequenceTitle, xLabel, yLabel, seriesLabels[0]);
	...
	private void processAllInputColumns() {
		...
		//if (selectedOption == JOptionPane.YES_OPTION) {
			...
			SequencePlotFrame pdf = new SequencePlotFrame(sequence1D, dataY, isLineVisible, "Poincare plot(s)", sequenceTitle, xLabel, yLabel, seriesLabels);
			Point pos = pdf.getLocation();
			pos.x = (int) (pos.getX() - 100);
			pos.y = (int) (pos.getY() + 100);
			pdf.setLocation(pos);
	...
	*/
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Plugin1DPoincarePlot.class, true);
	}
}
