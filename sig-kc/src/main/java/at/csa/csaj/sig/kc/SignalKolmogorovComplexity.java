...
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Kolmogorov complexity and LD ", weight = 330)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SignalKolmogorovComplexity<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class SignalKolmogorovComplexity<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	private Button buttonProcessAllColumns;
	...
	*/
	...
	private void generateTableHeader() {
		...
		tableOut.add(new GenericColumn("Signal range")); //"Entire signal", "Subsequent boxes", "Gliding box" 
		...
		if (choiceRadioButt_SignalRange.equals("Entire signal")){
			tableOut.add(new DoubleColumn("Signal size [kB]")); //0 SigSize, 1 KC, 2 SigSize-KC, 3 KC/SigSize, 4 numIter, 5 LD
			tableOut.add(new DoubleColumn("KC [kB]"));
			tableOut.add(new DoubleColumn("Signal size - KC [kB]"));
			tableOut.add(new DoubleColumn("KC/Signalsize"));
