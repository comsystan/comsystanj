/*-
 * #%L
 * Project: ImageJ2 signal plugin for computing fractal dimension with 1D Higuchi algorithm.
 * File: Plugin1DFractalDimensionHiguchi1D.java
 * 
 * $Id$
 * $HeadURL$
 * 
 ...
 * #L%
 */
package at.csa.csaj.plugin1d.frac.dim.hig1d;
...
import at.csa.csaj.plugin1d.frac.dim.hig1d.util.Higuchi;
...
@Plugin(type = ContextCommand.class, 
	...
	@Menu(label = "Higuchi dimension ", weight = 170)}) //Space at the end of the label is necessary to avoid duplicate with image2d plugin 
//public class SequenceFractalDimensionHiguchi1D<T extends RealType<T>> extends InteractiveCommand { // non blocking  GUI
public class Plugin1DFractalDimensionHiguchi1D<T extends RealType<T>> extends ContextCommand implements Previewable { //modal GUI with cancel
...
//	private Button buttonProcessAllColumns;
	...
	*/
	...
	private void generateTableHeader() {
		...
		if (choiceRadioButt_SequenceRange.equals("Entire sequence")){
			tableOut.add(new DoubleColumn("Dh"));	
			...
			if (choiceRadioButt_SurrogateType.equals("No surrogates")) {
				...
				tableOut.add(new DoubleColumn("Dh_Surr")); //Mean surrogate value	
				tableOut.add(new DoubleColumn("R2_Surr")); //Mean surrogate value
				tableOut.add(new DoubleColumn("StdErr_Surr")); //Mean surrogate value
				for (int s = 0; s < numSurrogates; s++) tableOut.add(new DoubleColumn("Dh_Surr-#"+(s+1))); 
			...
			for (int n = 1; n <= numSubsequentBoxes; n++) {
				tableOut.add(new DoubleColumn("Dh-#" + n));	
			}
			for (int n = 1; n <= numSubsequentBoxes; n++) {
...
//		}
	...
	public static void main(final String... args) throws Exception {
		...
		//open and run Plugin
		ij.command().run(Plugin1DFractalDimensionHiguchi1D.class, true);
	}
}
